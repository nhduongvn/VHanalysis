...
class HistoMaker:
    def __init__(self, samples, path, config, optionsList,GroupDict=None,filelist=[],mergeplot=False):
...
#=====>: apply cut to tree        
    ...
    def get_histos_from_tree(self,job,quick=True):
        ...
        input = ROOT.TFile.Open(self.tc.get_tree(job, '1'),'read')
...
#        print 'begin self.optionsList',self.optionsList
        ...
        for options in self.optionsList:
            ...
            treeCut = job.addtreecut + " && ("+treeCut+") && (" + job.specialCut + ")"
