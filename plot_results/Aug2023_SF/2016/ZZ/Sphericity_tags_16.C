#ifdef __CLING__
#pragma cling optimize(0)
#endif
void Sphericity_tags_16()
{
//=========Macro generated from canvas: Sphericity_tags_16/Sphericity_tags_16
//=========  (Fri Sep  1 13:23:33 2023) by ROOT version 6.28/04
   TCanvas *Sphericity_tags_16 = new TCanvas("Sphericity_tags_16", "Sphericity_tags_16",0,0,600,600);
   gStyle->SetOptStat(0);
   Sphericity_tags_16->SetHighLightColor(2);
   Sphericity_tags_16->Range(-0.2,-1.91091,1.133333,17.19819);
   Sphericity_tags_16->SetFillColor(0);
   Sphericity_tags_16->SetBorderMode(0);
   Sphericity_tags_16->SetBorderSize(2);
   Sphericity_tags_16->SetLeftMargin(0.15);
   Sphericity_tags_16->SetFrameBorderMode(0);
   Sphericity_tags_16->SetFrameBorderMode(0);
   
   TH1D *VbbHcc_tags_Sphericity__221 = new TH1D("VbbHcc_tags_Sphericity__221","",25,0,1);
   VbbHcc_tags_Sphericity__221->SetBinContent(1,14.55931);
   VbbHcc_tags_Sphericity__221->SetBinContent(2,9.630575);
   VbbHcc_tags_Sphericity__221->SetBinContent(3,4.434118);
   VbbHcc_tags_Sphericity__221->SetBinContent(4,3.706896);
   VbbHcc_tags_Sphericity__221->SetBinContent(5,1.708038);
   VbbHcc_tags_Sphericity__221->SetBinContent(6,2.013945);
   VbbHcc_tags_Sphericity__221->SetBinContent(8,0.2068406);
   VbbHcc_tags_Sphericity__221->SetBinContent(12,0.1960743);
   VbbHcc_tags_Sphericity__221->SetBinContent(16,0.1916587);
   VbbHcc_tags_Sphericity__221->SetBinError(1,1.818178);
   VbbHcc_tags_Sphericity__221->SetBinError(2,1.449675);
   VbbHcc_tags_Sphericity__221->SetBinError(3,0.9686933);
   VbbHcc_tags_Sphericity__221->SetBinError(4,0.9117812);
   VbbHcc_tags_Sphericity__221->SetBinError(5,0.6163901);
   VbbHcc_tags_Sphericity__221->SetBinError(6,0.6779777);
   VbbHcc_tags_Sphericity__221->SetBinError(8,0.2068406);
   VbbHcc_tags_Sphericity__221->SetBinError(12,0.1960743);
   VbbHcc_tags_Sphericity__221->SetBinError(16,0.1916587);
   VbbHcc_tags_Sphericity__221->SetEntries(173);

   Int_t ci;      // for color index setting
   TColor *color; // for color definition with alpha
   ci = TColor::GetColor("#33ff99");
   VbbHcc_tags_Sphericity__221->SetFillColor(ci);
   VbbHcc_tags_Sphericity__221->GetXaxis()->SetTitle("Sphericity");
   VbbHcc_tags_Sphericity__221->GetXaxis()->SetRange(1,25);
   VbbHcc_tags_Sphericity__221->GetXaxis()->SetLabelFont(42);
   VbbHcc_tags_Sphericity__221->GetXaxis()->SetTitleOffset(1);
   VbbHcc_tags_Sphericity__221->GetXaxis()->SetTitleFont(42);
   VbbHcc_tags_Sphericity__221->GetYaxis()->SetLabelFont(42);
   VbbHcc_tags_Sphericity__221->GetYaxis()->SetTitleFont(42);
   VbbHcc_tags_Sphericity__221->GetZaxis()->SetLabelFont(42);
   VbbHcc_tags_Sphericity__221->GetZaxis()->SetTitleOffset(1);
   VbbHcc_tags_Sphericity__221->GetZaxis()->SetTitleFont(42);
   VbbHcc_tags_Sphericity__221->Draw("HIST");
   TLatex *   tex = new TLatex(0.25,0.937775,"CMS Work in Progress #sqrt{s} = 13 TeV, 35.9 fb^{-1}");
   tex->SetNDC();
   tex->SetTextFont(42);
   tex->SetTextSize(0.04);
   tex->SetLineWidth(2);
   tex->Draw();
   Sphericity_tags_16->Modified();
   Sphericity_tags_16->cd();
   Sphericity_tags_16->SetSelected(Sphericity_tags_16);
}
