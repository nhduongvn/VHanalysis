#ifdef __CLING__
#pragma cling optimize(0)
#endif
void nJet_jets_17_logY()
{
//=========Macro generated from canvas: nJet_jets_17/nJet_jets_17
//=========  (Mon Dec 12 10:47:55 2022) by ROOT version 6.26/06
   TCanvas *nJet_jets_17 = new TCanvas("nJet_jets_17", "nJet_jets_17",0,0,600,600);
   nJet_jets_17->SetHighLightColor(2);
   nJet_jets_17->Range(-3.556941,-1.716665,15.90286,14.7145);
   nJet_jets_17->SetFillColor(0);
   nJet_jets_17->SetFillStyle(4000);
   nJet_jets_17->SetBorderMode(0);
   nJet_jets_17->SetBorderSize(2);
   nJet_jets_17->SetLogy();
   nJet_jets_17->SetLeftMargin(0.15709);
   nJet_jets_17->SetRightMargin(0.1234783);
   nJet_jets_17->SetBottomMargin(0.12);
   nJet_jets_17->SetFrameFillStyle(1000);
   nJet_jets_17->SetFrameBorderMode(0);
   nJet_jets_17->SetFrameFillStyle(1000);
   nJet_jets_17->SetFrameBorderMode(0);
   
   THStack *st = new THStack();
   st->SetName("st");
   st->SetTitle("");
   st->SetMinimum(12.1177);
   st->SetMaximum(3.578104e+12);
   
   TH1F *st_stack_2 = new TH1F("st_stack_2","",14,-0.5,13.5);
   st_stack_2->SetMinimum(1.799182);
   st_stack_2->SetMaximum(1.178644e+13);
   st_stack_2->SetDirectory(0);
   st_stack_2->SetStats(0);

   Int_t ci;      // for color index setting
   TColor *color; // for color definition with alpha
   ci = TColor::GetColor("#000099");
   st_stack_2->SetLineColor(ci);
   st_stack_2->GetXaxis()->SetTitle("Jet multiplicity");
   st_stack_2->GetXaxis()->SetRange(1,14);
   st_stack_2->GetXaxis()->SetLabelFont(42);
   st_stack_2->GetXaxis()->SetTitleOffset(1);
   st_stack_2->GetXaxis()->SetTitleFont(42);
   st_stack_2->GetYaxis()->SetTitle("Events/1.0");
   st_stack_2->GetYaxis()->SetLabelFont(42);
   st_stack_2->GetYaxis()->SetTitleSize(0.037);
   st_stack_2->GetYaxis()->SetTitleFont(42);
   st_stack_2->GetZaxis()->SetLabelFont(42);
   st_stack_2->GetZaxis()->SetTitleOffset(1);
   st_stack_2->GetZaxis()->SetTitleFont(42);
   st->SetHistogram(st_stack_2);
   
   
   TH1D *VbbHcc_jets_nJet_stack_1 = new TH1D("VbbHcc_jets_nJet_stack_1","",14,-0.5,13.5);
   VbbHcc_jets_nJet_stack_1->SetBinContent(1,1.251549e+09);
   VbbHcc_jets_nJet_stack_1->SetBinContent(2,1.188672e+10);
   VbbHcc_jets_nJet_stack_1->SetBinContent(3,3.576913e+10);
   VbbHcc_jets_nJet_stack_1->SetBinContent(4,2.693057e+10);
   VbbHcc_jets_nJet_stack_1->SetBinContent(5,9.453165e+09);
   VbbHcc_jets_nJet_stack_1->SetBinContent(6,1.988793e+09);
   VbbHcc_jets_nJet_stack_1->SetBinContent(7,3.678581e+08);
   VbbHcc_jets_nJet_stack_1->SetBinContent(8,6.52519e+07);
   VbbHcc_jets_nJet_stack_1->SetBinContent(9,1.14982e+07);
   VbbHcc_jets_nJet_stack_1->SetBinContent(10,1997474);
   VbbHcc_jets_nJet_stack_1->SetBinContent(11,339506.5);
   VbbHcc_jets_nJet_stack_1->SetBinContent(12,60575);
   VbbHcc_jets_nJet_stack_1->SetBinContent(13,9873.973);
   VbbHcc_jets_nJet_stack_1->SetBinContent(14,1524.9);
   VbbHcc_jets_nJet_stack_1->SetBinContent(15,407.5958);
   VbbHcc_jets_nJet_stack_1->SetBinError(1,1227746);
   VbbHcc_jets_nJet_stack_1->SetBinError(2,3788400);
   VbbHcc_jets_nJet_stack_1->SetBinError(3,6523419);
   VbbHcc_jets_nJet_stack_1->SetBinError(4,5475634);
   VbbHcc_jets_nJet_stack_1->SetBinError(5,3021610);
   VbbHcc_jets_nJet_stack_1->SetBinError(6,1227540);
   VbbHcc_jets_nJet_stack_1->SetBinError(7,449907.3);
   VbbHcc_jets_nJet_stack_1->SetBinError(8,157135.6);
   VbbHcc_jets_nJet_stack_1->SetBinError(9,53956.27);
   VbbHcc_jets_nJet_stack_1->SetBinError(10,18252.01);
   VbbHcc_jets_nJet_stack_1->SetBinError(11,5440.065);
   VbbHcc_jets_nJet_stack_1->SetBinError(12,2595.714);
   VbbHcc_jets_nJet_stack_1->SetBinError(13,542.8219);
   VbbHcc_jets_nJet_stack_1->SetBinError(14,111.4309);
   VbbHcc_jets_nJet_stack_1->SetBinError(15,58.16955);
   VbbHcc_jets_nJet_stack_1->SetEntries(2.186231e+08);

   ci = TColor::GetColor("#ff6600");
   VbbHcc_jets_nJet_stack_1->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_jets_nJet_stack_1->SetLineColor(ci);
   VbbHcc_jets_nJet_stack_1->GetXaxis()->SetRange(1,14);
   VbbHcc_jets_nJet_stack_1->GetXaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_1->GetXaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_1->GetXaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_1->GetYaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_1->GetYaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_1->GetZaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_1->GetZaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_1->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_jets_nJet_stack_1,"");
   
   TH1D *VbbHcc_jets_nJet_stack_2 = new TH1D("VbbHcc_jets_nJet_stack_2","",14,-0.5,13.5);
   VbbHcc_jets_nJet_stack_2->SetBinContent(1,604492);
   VbbHcc_jets_nJet_stack_2->SetBinContent(2,2332422);
   VbbHcc_jets_nJet_stack_2->SetBinContent(3,3734504);
   VbbHcc_jets_nJet_stack_2->SetBinContent(4,3314468);
   VbbHcc_jets_nJet_stack_2->SetBinContent(5,1880768);
   VbbHcc_jets_nJet_stack_2->SetBinContent(6,780403.6);
   VbbHcc_jets_nJet_stack_2->SetBinContent(7,270561.5);
   VbbHcc_jets_nJet_stack_2->SetBinContent(8,81349.34);
   VbbHcc_jets_nJet_stack_2->SetBinContent(9,21952.86);
   VbbHcc_jets_nJet_stack_2->SetBinContent(10,5342.516);
   VbbHcc_jets_nJet_stack_2->SetBinContent(11,1217.165);
   VbbHcc_jets_nJet_stack_2->SetBinContent(12,284.1484);
   VbbHcc_jets_nJet_stack_2->SetBinContent(13,57.89316);
   VbbHcc_jets_nJet_stack_2->SetBinContent(14,13.95593);
   VbbHcc_jets_nJet_stack_2->SetBinContent(15,3.384377);
   VbbHcc_jets_nJet_stack_2->SetBinError(1,229.0557);
   VbbHcc_jets_nJet_stack_2->SetBinError(2,473.028);
   VbbHcc_jets_nJet_stack_2->SetBinError(3,639.7684);
   VbbHcc_jets_nJet_stack_2->SetBinError(4,661.6825);
   VbbHcc_jets_nJet_stack_2->SetBinError(5,558.695);
   VbbHcc_jets_nJet_stack_2->SetBinError(6,399.2711);
   VbbHcc_jets_nJet_stack_2->SetBinError(7,247.4604);
   VbbHcc_jets_nJet_stack_2->SetBinError(8,138.0631);
   VbbHcc_jets_nJet_stack_2->SetBinError(9,71.82655);
   VbbHcc_jets_nJet_stack_2->SetBinError(10,35.03321);
   VbbHcc_jets_nJet_stack_2->SetBinError(11,16.42116);
   VbbHcc_jets_nJet_stack_2->SetBinError(12,7.93719);
   VbbHcc_jets_nJet_stack_2->SetBinError(13,3.441117);
   VbbHcc_jets_nJet_stack_2->SetBinError(14,1.708033);
   VbbHcc_jets_nJet_stack_2->SetBinError(15,0.8483954);
   VbbHcc_jets_nJet_stack_2->SetEntries(1.964522e+08);

   ci = TColor::GetColor("#660066");
   VbbHcc_jets_nJet_stack_2->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_jets_nJet_stack_2->SetLineColor(ci);
   VbbHcc_jets_nJet_stack_2->GetXaxis()->SetRange(1,14);
   VbbHcc_jets_nJet_stack_2->GetXaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_2->GetXaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_2->GetXaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_2->GetYaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_2->GetYaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_2->GetZaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_2->GetZaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_2->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_jets_nJet_stack_2,"");
   
   TH1D *VbbHcc_jets_nJet_stack_3 = new TH1D("VbbHcc_jets_nJet_stack_3","",14,-0.5,13.5);
   VbbHcc_jets_nJet_stack_3->SetBinContent(1,205246);
   VbbHcc_jets_nJet_stack_3->SetBinContent(2,1396654);
   VbbHcc_jets_nJet_stack_3->SetBinContent(3,4341472);
   VbbHcc_jets_nJet_stack_3->SetBinContent(4,7823540);
   VbbHcc_jets_nJet_stack_3->SetBinContent(5,9141333);
   VbbHcc_jets_nJet_stack_3->SetBinContent(6,7168285);
   VbbHcc_jets_nJet_stack_3->SetBinContent(7,3905029);
   VbbHcc_jets_nJet_stack_3->SetBinContent(8,1568436);
   VbbHcc_jets_nJet_stack_3->SetBinContent(9,519086.5);
   VbbHcc_jets_nJet_stack_3->SetBinContent(10,149793);
   VbbHcc_jets_nJet_stack_3->SetBinContent(11,39076.31);
   VbbHcc_jets_nJet_stack_3->SetBinContent(12,9477.052);
   VbbHcc_jets_nJet_stack_3->SetBinContent(13,2167.018);
   VbbHcc_jets_nJet_stack_3->SetBinContent(14,468.2063);
   VbbHcc_jets_nJet_stack_3->SetBinContent(15,118.9853);
   VbbHcc_jets_nJet_stack_3->SetBinError(1,100.4765);
   VbbHcc_jets_nJet_stack_3->SetBinError(2,266.4703);
   VbbHcc_jets_nJet_stack_3->SetBinError(3,488.7025);
   VbbHcc_jets_nJet_stack_3->SetBinError(4,691.4404);
   VbbHcc_jets_nJet_stack_3->SetBinError(5,786.5539);
   VbbHcc_jets_nJet_stack_3->SetBinError(6,723.7539);
   VbbHcc_jets_nJet_stack_3->SetBinError(7,544.8136);
   VbbHcc_jets_nJet_stack_3->SetBinError(8,348.137);
   VbbHcc_jets_nJet_stack_3->SetBinError(9,201.0592);
   VbbHcc_jets_nJet_stack_3->SetBinError(10,108.2096);
   VbbHcc_jets_nJet_stack_3->SetBinError(11,55.31702);
   VbbHcc_jets_nJet_stack_3->SetBinError(12,27.23721);
   VbbHcc_jets_nJet_stack_3->SetBinError(13,13.00826);
   VbbHcc_jets_nJet_stack_3->SetBinError(14,6.052514);
   VbbHcc_jets_nJet_stack_3->SetBinError(15,3.050489);
   VbbHcc_jets_nJet_stack_3->SetEntries(6.16706e+08);

   ci = TColor::GetColor("#cc00cc");
   VbbHcc_jets_nJet_stack_3->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_jets_nJet_stack_3->SetLineColor(ci);
   VbbHcc_jets_nJet_stack_3->GetXaxis()->SetRange(1,14);
   VbbHcc_jets_nJet_stack_3->GetXaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_3->GetXaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_3->GetXaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_3->GetYaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_3->GetYaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_3->GetZaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_3->GetZaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_3->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_jets_nJet_stack_3,"");
   
   TH1D *VbbHcc_jets_nJet_stack_4 = new TH1D("VbbHcc_jets_nJet_stack_4","",14,-0.5,13.5);
   VbbHcc_jets_nJet_stack_4->SetBinContent(1,7196.772);
   VbbHcc_jets_nJet_stack_4->SetBinContent(2,110487.9);
   VbbHcc_jets_nJet_stack_4->SetBinContent(3,692961.8);
   VbbHcc_jets_nJet_stack_4->SetBinContent(4,1606969);
   VbbHcc_jets_nJet_stack_4->SetBinContent(5,1519495);
   VbbHcc_jets_nJet_stack_4->SetBinContent(6,789795.8);
   VbbHcc_jets_nJet_stack_4->SetBinContent(7,262300.3);
   VbbHcc_jets_nJet_stack_4->SetBinContent(8,60181.43);
   VbbHcc_jets_nJet_stack_4->SetBinContent(9,11060.87);
   VbbHcc_jets_nJet_stack_4->SetBinContent(10,1675.421);
   VbbHcc_jets_nJet_stack_4->SetBinContent(11,242.6286);
   VbbHcc_jets_nJet_stack_4->SetBinContent(12,26.90448);
   VbbHcc_jets_nJet_stack_4->SetBinContent(13,3.295318);
   VbbHcc_jets_nJet_stack_4->SetBinContent(14,0.8408019);
   VbbHcc_jets_nJet_stack_4->SetBinError(1,45.12255);
   VbbHcc_jets_nJet_stack_4->SetBinError(2,194.5459);
   VbbHcc_jets_nJet_stack_4->SetBinError(3,526.7198);
   VbbHcc_jets_nJet_stack_4->SetBinError(4,819.9704);
   VbbHcc_jets_nJet_stack_4->SetBinError(5,802.3813);
   VbbHcc_jets_nJet_stack_4->SetBinError(6,579.8986);
   VbbHcc_jets_nJet_stack_4->SetBinError(7,334.4762);
   VbbHcc_jets_nJet_stack_4->SetBinError(8,160.1342);
   VbbHcc_jets_nJet_stack_4->SetBinError(9,68.58598);
   VbbHcc_jets_nJet_stack_4->SetBinError(10,26.70977);
   VbbHcc_jets_nJet_stack_4->SetBinError(11,10.12496);
   VbbHcc_jets_nJet_stack_4->SetBinError(12,3.326134);
   VbbHcc_jets_nJet_stack_4->SetBinError(13,1.17718);
   VbbHcc_jets_nJet_stack_4->SetBinError(14,0.5947338);
   VbbHcc_jets_nJet_stack_4->SetEntries(1.282701e+07);

   ci = TColor::GetColor("#00cccc");
   VbbHcc_jets_nJet_stack_4->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_jets_nJet_stack_4->SetLineColor(ci);
   VbbHcc_jets_nJet_stack_4->GetXaxis()->SetRange(1,14);
   VbbHcc_jets_nJet_stack_4->GetXaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_4->GetXaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_4->GetXaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_4->GetYaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_4->GetYaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_4->GetZaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_4->GetZaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_4->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_jets_nJet_stack_4,"");
   
   TH1D *VbbHcc_jets_nJet_stack_5 = new TH1D("VbbHcc_jets_nJet_stack_5","",14,-0.5,13.5);
   VbbHcc_jets_nJet_stack_5->SetBinContent(1,18938.9);
   VbbHcc_jets_nJet_stack_5->SetBinContent(2,287162.2);
   VbbHcc_jets_nJet_stack_5->SetBinContent(3,1689948);
   VbbHcc_jets_nJet_stack_5->SetBinContent(4,3919200);
   VbbHcc_jets_nJet_stack_5->SetBinContent(5,3674904);
   VbbHcc_jets_nJet_stack_5->SetBinContent(6,1842260);
   VbbHcc_jets_nJet_stack_5->SetBinContent(7,579118.3);
   VbbHcc_jets_nJet_stack_5->SetBinContent(8,137032.4);
   VbbHcc_jets_nJet_stack_5->SetBinContent(9,26666.62);
   VbbHcc_jets_nJet_stack_5->SetBinContent(10,4421.671);
   VbbHcc_jets_nJet_stack_5->SetBinContent(11,618.6614);
   VbbHcc_jets_nJet_stack_5->SetBinContent(12,83.3123);
   VbbHcc_jets_nJet_stack_5->SetBinContent(13,13.852);
   VbbHcc_jets_nJet_stack_5->SetBinContent(14,2.292169);
   VbbHcc_jets_nJet_stack_5->SetBinError(1,128.4563);
   VbbHcc_jets_nJet_stack_5->SetBinError(2,552.8203);
   VbbHcc_jets_nJet_stack_5->SetBinError(3,1450.625);
   VbbHcc_jets_nJet_stack_5->SetBinError(4,2263.561);
   VbbHcc_jets_nJet_stack_5->SetBinError(5,2207.59);
   VbbHcc_jets_nJet_stack_5->SetBinError(6,1566.56);
   VbbHcc_jets_nJet_stack_5->SetBinError(7,878.587);
   VbbHcc_jets_nJet_stack_5->SetBinError(8,427.4125);
   VbbHcc_jets_nJet_stack_5->SetBinError(9,188.149);
   VbbHcc_jets_nJet_stack_5->SetBinError(10,76.60559);
   VbbHcc_jets_nJet_stack_5->SetBinError(11,28.72826);
   VbbHcc_jets_nJet_stack_5->SetBinError(12,10.56493);
   VbbHcc_jets_nJet_stack_5->SetBinError(13,4.336574);
   VbbHcc_jets_nJet_stack_5->SetBinError(14,1.677532);
   VbbHcc_jets_nJet_stack_5->SetEntries(9908404);

   ci = TColor::GetColor("#ff99cc");
   VbbHcc_jets_nJet_stack_5->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_jets_nJet_stack_5->SetLineColor(ci);
   VbbHcc_jets_nJet_stack_5->GetXaxis()->SetRange(1,14);
   VbbHcc_jets_nJet_stack_5->GetXaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_5->GetXaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_5->GetXaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_5->GetYaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_5->GetYaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_5->GetZaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_5->GetZaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_5->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_jets_nJet_stack_5,"");
   
   TH1D *VbbHcc_jets_nJet_stack_6 = new TH1D("VbbHcc_jets_nJet_stack_6","",14,-0.5,13.5);
   VbbHcc_jets_nJet_stack_6->SetBinContent(1,664587.6);
   VbbHcc_jets_nJet_stack_6->SetBinContent(2,1031739);
   VbbHcc_jets_nJet_stack_6->SetBinContent(3,952225.7);
   VbbHcc_jets_nJet_stack_6->SetBinContent(4,534808);
   VbbHcc_jets_nJet_stack_6->SetBinContent(5,205852.7);
   VbbHcc_jets_nJet_stack_6->SetBinContent(6,53954.79);
   VbbHcc_jets_nJet_stack_6->SetBinContent(7,11946.9);
   VbbHcc_jets_nJet_stack_6->SetBinContent(8,2445.067);
   VbbHcc_jets_nJet_stack_6->SetBinContent(9,452.5835);
   VbbHcc_jets_nJet_stack_6->SetBinContent(10,84.37188);
   VbbHcc_jets_nJet_stack_6->SetBinContent(11,13.76949);
   VbbHcc_jets_nJet_stack_6->SetBinContent(12,1.490395);
   VbbHcc_jets_nJet_stack_6->SetBinContent(13,0.6792264);
   VbbHcc_jets_nJet_stack_6->SetBinError(1,420.972);
   VbbHcc_jets_nJet_stack_6->SetBinError(2,525.2035);
   VbbHcc_jets_nJet_stack_6->SetBinError(3,505.2654);
   VbbHcc_jets_nJet_stack_6->SetBinError(4,378.855);
   VbbHcc_jets_nJet_stack_6->SetBinError(5,234.9891);
   VbbHcc_jets_nJet_stack_6->SetBinError(6,120.1709);
   VbbHcc_jets_nJet_stack_6->SetBinError(7,56.47023);
   VbbHcc_jets_nJet_stack_6->SetBinError(8,25.56742);
   VbbHcc_jets_nJet_stack_6->SetBinError(9,10.9087);
   VbbHcc_jets_nJet_stack_6->SetBinError(10,4.681502);
   VbbHcc_jets_nJet_stack_6->SetBinError(11,1.900635);
   VbbHcc_jets_nJet_stack_6->SetBinError(12,0.621972);
   VbbHcc_jets_nJet_stack_6->SetBinError(13,0.4047725);
   VbbHcc_jets_nJet_stack_6->SetEntries(1.347846e+07);

   ci = TColor::GetColor("#9933ff");
   VbbHcc_jets_nJet_stack_6->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_jets_nJet_stack_6->SetLineColor(ci);
   VbbHcc_jets_nJet_stack_6->GetXaxis()->SetRange(1,14);
   VbbHcc_jets_nJet_stack_6->GetXaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_6->GetXaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_6->GetXaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_6->GetYaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_6->GetYaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_6->GetZaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_6->GetZaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_6->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_jets_nJet_stack_6,"");
   
   TH1D *VbbHcc_jets_nJet_stack_7 = new TH1D("VbbHcc_jets_nJet_stack_7","",14,-0.5,13.5);
   VbbHcc_jets_nJet_stack_7->SetBinContent(1,220005.2);
   VbbHcc_jets_nJet_stack_7->SetBinContent(2,362473.1);
   VbbHcc_jets_nJet_stack_7->SetBinContent(3,347604.4);
   VbbHcc_jets_nJet_stack_7->SetBinContent(4,202022.6);
   VbbHcc_jets_nJet_stack_7->SetBinContent(5,84768.68);
   VbbHcc_jets_nJet_stack_7->SetBinContent(6,23638.59);
   VbbHcc_jets_nJet_stack_7->SetBinContent(7,5457.666);
   VbbHcc_jets_nJet_stack_7->SetBinContent(8,1115.17);
   VbbHcc_jets_nJet_stack_7->SetBinContent(9,211.7918);
   VbbHcc_jets_nJet_stack_7->SetBinContent(10,35.73832);
   VbbHcc_jets_nJet_stack_7->SetBinContent(11,6.662024);
   VbbHcc_jets_nJet_stack_7->SetBinContent(12,1.649437);
   VbbHcc_jets_nJet_stack_7->SetBinContent(13,0.2845963);
   VbbHcc_jets_nJet_stack_7->SetBinError(1,192.7107);
   VbbHcc_jets_nJet_stack_7->SetBinError(2,247.4004);
   VbbHcc_jets_nJet_stack_7->SetBinError(3,242.3614);
   VbbHcc_jets_nJet_stack_7->SetBinError(4,184.7895);
   VbbHcc_jets_nJet_stack_7->SetBinError(5,119.7181);
   VbbHcc_jets_nJet_stack_7->SetBinError(6,63.12499);
   VbbHcc_jets_nJet_stack_7->SetBinError(7,30.29475);
   VbbHcc_jets_nJet_stack_7->SetBinError(8,13.66246);
   VbbHcc_jets_nJet_stack_7->SetBinError(9,5.95356);
   VbbHcc_jets_nJet_stack_7->SetBinError(10,2.441589);
   VbbHcc_jets_nJet_stack_7->SetBinError(11,1.028915);
   VbbHcc_jets_nJet_stack_7->SetBinError(12,0.5301737);
   VbbHcc_jets_nJet_stack_7->SetBinError(13,0.2073912);
   VbbHcc_jets_nJet_stack_7->SetEntries(7702296);

   ci = TColor::GetColor("#3399ff");
   VbbHcc_jets_nJet_stack_7->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_jets_nJet_stack_7->SetLineColor(ci);
   VbbHcc_jets_nJet_stack_7->GetXaxis()->SetRange(1,14);
   VbbHcc_jets_nJet_stack_7->GetXaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_7->GetXaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_7->GetXaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_7->GetYaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_7->GetYaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_7->GetZaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_7->GetZaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_7->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_jets_nJet_stack_7,"");
   
   TH1D *VbbHcc_jets_nJet_stack_8 = new TH1D("VbbHcc_jets_nJet_stack_8","",14,-0.5,13.5);
   VbbHcc_jets_nJet_stack_8->SetBinContent(1,100644.8);
   VbbHcc_jets_nJet_stack_8->SetBinContent(2,144689.3);
   VbbHcc_jets_nJet_stack_8->SetBinContent(3,152073.6);
   VbbHcc_jets_nJet_stack_8->SetBinContent(4,96349.84);
   VbbHcc_jets_nJet_stack_8->SetBinContent(5,42158.23);
   VbbHcc_jets_nJet_stack_8->SetBinContent(6,11258.72);
   VbbHcc_jets_nJet_stack_8->SetBinContent(7,2482.861);
   VbbHcc_jets_nJet_stack_8->SetBinContent(8,491.2734);
   VbbHcc_jets_nJet_stack_8->SetBinContent(9,79.05607);
   VbbHcc_jets_nJet_stack_8->SetBinContent(10,18.44269);
   VbbHcc_jets_nJet_stack_8->SetBinContent(11,1.607326);
   VbbHcc_jets_nJet_stack_8->SetBinContent(13,0.5282545);
   VbbHcc_jets_nJet_stack_8->SetBinError(1,215.0713);
   VbbHcc_jets_nJet_stack_8->SetBinError(2,257.7902);
   VbbHcc_jets_nJet_stack_8->SetBinError(3,264.7042);
   VbbHcc_jets_nJet_stack_8->SetBinError(4,210.7147);
   VbbHcc_jets_nJet_stack_8->SetBinError(5,139.3977);
   VbbHcc_jets_nJet_stack_8->SetBinError(6,71.99891);
   VbbHcc_jets_nJet_stack_8->SetBinError(7,33.80371);
   VbbHcc_jets_nJet_stack_8->SetBinError(8,15.03731);
   VbbHcc_jets_nJet_stack_8->SetBinError(9,5.93409);
   VbbHcc_jets_nJet_stack_8->SetBinError(10,2.856245);
   VbbHcc_jets_nJet_stack_8->SetBinError(11,0.9317198);
   VbbHcc_jets_nJet_stack_8->SetBinError(13,0.5282545);
   VbbHcc_jets_nJet_stack_8->SetEntries(1246544);

   ci = TColor::GetColor("#33ff99");
   VbbHcc_jets_nJet_stack_8->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_jets_nJet_stack_8->SetLineColor(ci);
   VbbHcc_jets_nJet_stack_8->GetXaxis()->SetRange(1,14);
   VbbHcc_jets_nJet_stack_8->GetXaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_8->GetXaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_8->GetXaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_8->GetYaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_8->GetYaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_8->GetZaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_8->GetZaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_8->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_jets_nJet_stack_8,"");
   
   TH1D *VbbHcc_jets_nJet_stack_9 = new TH1D("VbbHcc_jets_nJet_stack_9","",14,-0.5,13.5);
   VbbHcc_jets_nJet_stack_9->SetBinContent(1,728.6841);
   VbbHcc_jets_nJet_stack_9->SetBinContent(2,1817.046);
   VbbHcc_jets_nJet_stack_9->SetBinContent(3,3775.656);
   VbbHcc_jets_nJet_stack_9->SetBinContent(4,4881.89);
   VbbHcc_jets_nJet_stack_9->SetBinContent(5,3450.034);
   VbbHcc_jets_nJet_stack_9->SetBinContent(6,1295.547);
   VbbHcc_jets_nJet_stack_9->SetBinContent(7,414.9002);
   VbbHcc_jets_nJet_stack_9->SetBinContent(8,122.5856);
   VbbHcc_jets_nJet_stack_9->SetBinContent(9,33.78059);
   VbbHcc_jets_nJet_stack_9->SetBinContent(10,8.861768);
   VbbHcc_jets_nJet_stack_9->SetBinContent(11,2.264479);
   VbbHcc_jets_nJet_stack_9->SetBinContent(12,0.5493148);
   VbbHcc_jets_nJet_stack_9->SetBinContent(13,0.1304666);
   VbbHcc_jets_nJet_stack_9->SetBinContent(14,0.03236824);
   VbbHcc_jets_nJet_stack_9->SetBinContent(15,0.01016319);
   VbbHcc_jets_nJet_stack_9->SetBinError(1,1.260611);
   VbbHcc_jets_nJet_stack_9->SetBinError(2,1.935818);
   VbbHcc_jets_nJet_stack_9->SetBinError(3,2.754974);
   VbbHcc_jets_nJet_stack_9->SetBinError(4,3.110196);
   VbbHcc_jets_nJet_stack_9->SetBinError(5,2.594309);
   VbbHcc_jets_nJet_stack_9->SetBinError(6,1.567513);
   VbbHcc_jets_nJet_stack_9->SetBinError(7,0.8824885);
   VbbHcc_jets_nJet_stack_9->SetBinError(8,0.4782138);
   VbbHcc_jets_nJet_stack_9->SetBinError(9,0.2505737);
   VbbHcc_jets_nJet_stack_9->SetBinError(10,0.127836);
   VbbHcc_jets_nJet_stack_9->SetBinError(11,0.06460884);
   VbbHcc_jets_nJet_stack_9->SetBinError(12,0.0318661);
   VbbHcc_jets_nJet_stack_9->SetBinError(13,0.01569208);
   VbbHcc_jets_nJet_stack_9->SetBinError(14,0.007729775);
   VbbHcc_jets_nJet_stack_9->SetBinError(15,0.004241009);
   VbbHcc_jets_nJet_stack_9->SetEntries(9814626);

   ci = TColor::GetColor("#cccc00");
   VbbHcc_jets_nJet_stack_9->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_jets_nJet_stack_9->SetLineColor(ci);
   VbbHcc_jets_nJet_stack_9->GetXaxis()->SetRange(1,14);
   VbbHcc_jets_nJet_stack_9->GetXaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_9->GetXaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_9->GetXaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_9->GetYaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_9->GetYaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_9->GetZaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_9->GetZaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_9->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_jets_nJet_stack_9,"");
   
   TH1D *VbbHcc_jets_nJet_stack_10 = new TH1D("VbbHcc_jets_nJet_stack_10","",14,-0.5,13.5);
   VbbHcc_jets_nJet_stack_10->SetBinContent(1,5.225075);
   VbbHcc_jets_nJet_stack_10->SetBinContent(2,44.98504);
   VbbHcc_jets_nJet_stack_10->SetBinContent(3,224.9511);
   VbbHcc_jets_nJet_stack_10->SetBinContent(4,551.0572);
   VbbHcc_jets_nJet_stack_10->SetBinContent(5,680.2396);
   VbbHcc_jets_nJet_stack_10->SetBinContent(6,421.7644);
   VbbHcc_jets_nJet_stack_10->SetBinContent(7,173.1077);
   VbbHcc_jets_nJet_stack_10->SetBinContent(8,55.72863);
   VbbHcc_jets_nJet_stack_10->SetBinContent(9,15.21086);
   VbbHcc_jets_nJet_stack_10->SetBinContent(10,3.732488);
   VbbHcc_jets_nJet_stack_10->SetBinContent(11,0.8577502);
   VbbHcc_jets_nJet_stack_10->SetBinContent(12,0.1881563);
   VbbHcc_jets_nJet_stack_10->SetBinContent(13,0.03496506);
   VbbHcc_jets_nJet_stack_10->SetBinContent(14,0.009110245);
   VbbHcc_jets_nJet_stack_10->SetBinContent(15,0.0008601192);
   VbbHcc_jets_nJet_stack_10->SetBinError(1,0.04746917);
   VbbHcc_jets_nJet_stack_10->SetBinError(2,0.1398054);
   VbbHcc_jets_nJet_stack_10->SetBinError(3,0.3153702);
   VbbHcc_jets_nJet_stack_10->SetBinError(4,0.4953975);
   VbbHcc_jets_nJet_stack_10->SetBinError(5,0.5508855);
   VbbHcc_jets_nJet_stack_10->SetBinError(6,0.4334713);
   VbbHcc_jets_nJet_stack_10->SetBinError(7,0.2774045);
   VbbHcc_jets_nJet_stack_10->SetBinError(8,0.1571907);
   VbbHcc_jets_nJet_stack_10->SetBinError(9,0.0820263);
   VbbHcc_jets_nJet_stack_10->SetBinError(10,0.04057864);
   VbbHcc_jets_nJet_stack_10->SetBinError(11,0.01945918);
   VbbHcc_jets_nJet_stack_10->SetBinError(12,0.009073303);
   VbbHcc_jets_nJet_stack_10->SetBinError(13,0.003893344);
   VbbHcc_jets_nJet_stack_10->SetBinError(14,0.001987419);
   VbbHcc_jets_nJet_stack_10->SetBinError(15,0.0006082013);
   VbbHcc_jets_nJet_stack_10->SetEntries(4977633);

   ci = TColor::GetColor("#0000cc");
   VbbHcc_jets_nJet_stack_10->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_jets_nJet_stack_10->SetLineColor(ci);
   VbbHcc_jets_nJet_stack_10->GetXaxis()->SetRange(1,14);
   VbbHcc_jets_nJet_stack_10->GetXaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_10->GetXaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_10->GetXaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_10->GetYaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_10->GetYaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_10->GetZaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_10->GetZaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_10->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_jets_nJet_stack_10,"");
   
   TH1D *VbbHcc_jets_nJet_stack_11 = new TH1D("VbbHcc_jets_nJet_stack_11","",14,-0.5,13.5);
   VbbHcc_jets_nJet_stack_11->SetBinContent(1,27.65204);
   VbbHcc_jets_nJet_stack_11->SetBinContent(2,70.83744);
   VbbHcc_jets_nJet_stack_11->SetBinContent(3,152.9184);
   VbbHcc_jets_nJet_stack_11->SetBinContent(4,214.2052);
   VbbHcc_jets_nJet_stack_11->SetBinContent(5,162.4626);
   VbbHcc_jets_nJet_stack_11->SetBinContent(6,62.26793);
   VbbHcc_jets_nJet_stack_11->SetBinContent(7,20.09871);
   VbbHcc_jets_nJet_stack_11->SetBinContent(8,6.180149);
   VbbHcc_jets_nJet_stack_11->SetBinContent(9,1.650179);
   VbbHcc_jets_nJet_stack_11->SetBinContent(10,0.3915146);
   VbbHcc_jets_nJet_stack_11->SetBinContent(11,0.1216632);
   VbbHcc_jets_nJet_stack_11->SetBinContent(12,0.0253551);
   VbbHcc_jets_nJet_stack_11->SetBinContent(13,0.009327633);
   VbbHcc_jets_nJet_stack_11->SetBinError(1,0.2361144);
   VbbHcc_jets_nJet_stack_11->SetBinError(2,0.3703217);
   VbbHcc_jets_nJet_stack_11->SetBinError(3,0.536431);
   VbbHcc_jets_nJet_stack_11->SetBinError(4,0.6306065);
   VbbHcc_jets_nJet_stack_11->SetBinError(5,0.5454039);
   VbbHcc_jets_nJet_stack_11->SetBinError(6,0.3326931);
   VbbHcc_jets_nJet_stack_11->SetBinError(7,0.1876949);
   VbbHcc_jets_nJet_stack_11->SetBinError(8,0.1040305);
   VbbHcc_jets_nJet_stack_11->SetBinError(9,0.0533637);
   VbbHcc_jets_nJet_stack_11->SetBinError(10,0.02571355);
   VbbHcc_jets_nJet_stack_11->SetBinError(11,0.01466117);
   VbbHcc_jets_nJet_stack_11->SetBinError(12,0.006862831);
   VbbHcc_jets_nJet_stack_11->SetBinError(13,0.004138877);
   VbbHcc_jets_nJet_stack_11->SetEntries(495304);

   ci = TColor::GetColor("#cc0000");
   VbbHcc_jets_nJet_stack_11->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_jets_nJet_stack_11->SetLineColor(ci);
   VbbHcc_jets_nJet_stack_11->GetXaxis()->SetRange(1,14);
   VbbHcc_jets_nJet_stack_11->GetXaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_11->GetXaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_11->GetXaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_11->GetYaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_11->GetYaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_11->GetZaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_11->GetZaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_11->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_jets_nJet_stack_11,"");
   
   TH1D *VbbHcc_jets_nJet_stack_12 = new TH1D("VbbHcc_jets_nJet_stack_12","",14,-0.5,13.5);
   VbbHcc_jets_nJet_stack_12->SetBinContent(1,0.2071104);
   VbbHcc_jets_nJet_stack_12->SetBinContent(2,1.769448);
   VbbHcc_jets_nJet_stack_12->SetBinContent(3,9.079821);
   VbbHcc_jets_nJet_stack_12->SetBinContent(4,24.21543);
   VbbHcc_jets_nJet_stack_12->SetBinContent(5,32.17193);
   VbbHcc_jets_nJet_stack_12->SetBinContent(6,20.29133);
   VbbHcc_jets_nJet_stack_12->SetBinContent(7,8.245401);
   VbbHcc_jets_nJet_stack_12->SetBinContent(8,2.559333);
   VbbHcc_jets_nJet_stack_12->SetBinContent(9,0.6861844);
   VbbHcc_jets_nJet_stack_12->SetBinContent(10,0.1573423);
   VbbHcc_jets_nJet_stack_12->SetBinContent(11,0.03442011);
   VbbHcc_jets_nJet_stack_12->SetBinContent(12,0.00664898);
   VbbHcc_jets_nJet_stack_12->SetBinContent(13,0.00102282);
   VbbHcc_jets_nJet_stack_12->SetBinError(1,0.006879194);
   VbbHcc_jets_nJet_stack_12->SetBinError(2,0.02024997);
   VbbHcc_jets_nJet_stack_12->SetBinError(3,0.04670727);
   VbbHcc_jets_nJet_stack_12->SetBinError(4,0.07687459);
   VbbHcc_jets_nJet_stack_12->SetBinError(5,0.08872847);
   VbbHcc_jets_nJet_stack_12->SetBinError(6,0.07037927);
   VbbHcc_jets_nJet_stack_12->SetBinError(7,0.04477624);
   VbbHcc_jets_nJet_stack_12->SetBinError(8,0.0248786);
   VbbHcc_jets_nJet_stack_12->SetBinError(9,0.01284066);
   VbbHcc_jets_nJet_stack_12->SetBinError(10,0.006136168);
   VbbHcc_jets_nJet_stack_12->SetBinError(11,0.002893265);
   VbbHcc_jets_nJet_stack_12->SetBinError(12,0.00121842);
   VbbHcc_jets_nJet_stack_12->SetBinError(13,0.0004486212);
   VbbHcc_jets_nJet_stack_12->SetEntries(465395);

   ci = TColor::GetColor("#00cc00");
   VbbHcc_jets_nJet_stack_12->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_jets_nJet_stack_12->SetLineColor(ci);
   VbbHcc_jets_nJet_stack_12->GetXaxis()->SetRange(1,14);
   VbbHcc_jets_nJet_stack_12->GetXaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_12->GetXaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_12->GetXaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_12->GetYaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_12->GetYaxis()->SetTitleFont(42);
   VbbHcc_jets_nJet_stack_12->GetZaxis()->SetLabelFont(42);
   VbbHcc_jets_nJet_stack_12->GetZaxis()->SetTitleOffset(1);
   VbbHcc_jets_nJet_stack_12->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_jets_nJet_stack_12,"");
   st->Draw("hist");
   
   TLegend *leg = new TLegend(0.48,0.62,0.85,0.87,NULL,"brNDC");
   leg->SetBorderSize(0);
   leg->SetTextSize(0.035);
   leg->SetLineColor(1);
   leg->SetLineStyle(1);
   leg->SetLineWidth(2);
   leg->SetFillColor(0);
   leg->SetFillStyle(1001);
   TLegendEntry *entry=leg->AddEntry("VbbHcc_jets_nJet_stack_12","ggZHcc","F");

   ci = TColor::GetColor("#00cc00");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_jets_nJet_stack_11","ZHcc","F");

   ci = TColor::GetColor("#cc0000");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_jets_nJet_stack_10","ggZHbb","F");

   ci = TColor::GetColor("#0000cc");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_jets_nJet_stack_9","ZHbb","F");

   ci = TColor::GetColor("#cccc00");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_jets_nJet_stack_8","ZZ","F");

   ci = TColor::GetColor("#33ff99");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_jets_nJet_stack_7","WZ","F");

   ci = TColor::GetColor("#3399ff");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_jets_nJet_stack_6","WW","F");

   ci = TColor::GetColor("#9933ff");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_jets_nJet_stack_5","W + jets","F");

   ci = TColor::GetColor("#ff99cc");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_jets_nJet_stack_4","Z + jets","F");

   ci = TColor::GetColor("#00cccc");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_jets_nJet_stack_3","t#bar{t}","F");

   ci = TColor::GetColor("#cc00cc");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_jets_nJet_stack_2","Single Top","F");

   ci = TColor::GetColor("#660066");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_jets_nJet_stack_1","QCD","F");

   ci = TColor::GetColor("#ff6600");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   leg->Draw();
   TLatex *   tex = new TLatex(0.25,0.937775,"CMS Work in Progress #sqrt{s} = 13 TeV, 41.5 fb^{-1}");
tex->SetNDC();
   tex->SetTextFont(42);
   tex->SetTextSize(0.04);
   tex->SetLineWidth(2);
   tex->Draw();
   nJet_jets_17->Modified();
   nJet_jets_17->cd();
   nJet_jets_17->SetSelected(nJet_jets_17);
}
