#ifdef __CLING__
#pragma cling optimize(0)
#endif
void H_pt_tags_17_logY()
{
//=========Macro generated from canvas: H_pt_tags_17/H_pt_tags_17
//=========  (Tue Dec  6 14:34:30 2022) by ROOT version 6.26/06
   TCanvas *H_pt_tags_17 = new TCanvas("H_pt_tags_17", "H_pt_tags_17",0,0,600,600);
   H_pt_tags_17->SetHighLightColor(2);
   H_pt_tags_17->Range(37.97653,-3.394789,1705.96,10.92885);
   H_pt_tags_17->SetFillColor(0);
   H_pt_tags_17->SetFillStyle(4000);
   H_pt_tags_17->SetBorderMode(0);
   H_pt_tags_17->SetBorderSize(2);
   H_pt_tags_17->SetLogy();
   H_pt_tags_17->SetLeftMargin(0.15709);
   H_pt_tags_17->SetRightMargin(0.1234783);
   H_pt_tags_17->SetBottomMargin(0.12);
   H_pt_tags_17->SetFrameFillStyle(1000);
   H_pt_tags_17->SetFrameBorderMode(0);
   H_pt_tags_17->SetFrameFillStyle(1000);
   H_pt_tags_17->SetFrameBorderMode(0);
   
   THStack *st = new THStack();
   st->SetName("st");
   st->SetTitle("");
   st->SetMinimum(0.1257117);
   st->SetMaximum(1.05105e+09);
   
   TH1F *st_stack_18 = new TH1F("st_stack_18","",40,0,2000);
   st_stack_18->SetMinimum(0.02108859);
   st_stack_18->SetMaximum(3.136806e+09);
   st_stack_18->SetDirectory(0);
   st_stack_18->SetStats(0);

   Int_t ci;      // for color index setting
   TColor *color; // for color definition with alpha
   ci = TColor::GetColor("#000099");
   st_stack_18->SetLineColor(ci);
   st_stack_18->GetXaxis()->SetTitle("H p_{T} [GeV]");
   st_stack_18->GetXaxis()->SetRange(7,30);
   st_stack_18->GetXaxis()->SetLabelFont(42);
   st_stack_18->GetXaxis()->SetTitleOffset(1);
   st_stack_18->GetXaxis()->SetTitleFont(42);
   st_stack_18->GetYaxis()->SetTitle("Events/50.0");
   st_stack_18->GetYaxis()->SetLabelFont(42);
   st_stack_18->GetYaxis()->SetTitleSize(0.037);
   st_stack_18->GetYaxis()->SetTitleFont(42);
   st_stack_18->GetZaxis()->SetLabelFont(42);
   st_stack_18->GetZaxis()->SetTitleOffset(1);
   st_stack_18->GetZaxis()->SetTitleFont(42);
   st->SetHistogram(st_stack_18);
   
   
   TH1D *VbbHcc_tags_H_pt_stack_1 = new TH1D("VbbHcc_tags_H_pt_stack_1","",40,0,2000);
   VbbHcc_tags_H_pt_stack_1->SetBinContent(1,6783627);
   VbbHcc_tags_H_pt_stack_1->SetBinContent(2,1.05105e+07);
   VbbHcc_tags_H_pt_stack_1->SetBinContent(3,3674094);
   VbbHcc_tags_H_pt_stack_1->SetBinContent(4,281681.4);
   VbbHcc_tags_H_pt_stack_1->SetBinContent(5,18443.22);
   VbbHcc_tags_H_pt_stack_1->SetBinContent(6,1429.655);
   VbbHcc_tags_H_pt_stack_1->SetBinContent(7,1066.695);
   VbbHcc_tags_H_pt_stack_1->SetBinError(1,95318.34);
   VbbHcc_tags_H_pt_stack_1->SetBinError(2,118444.4);
   VbbHcc_tags_H_pt_stack_1->SetBinError(3,70002.57);
   VbbHcc_tags_H_pt_stack_1->SetBinError(4,19392.06);
   VbbHcc_tags_H_pt_stack_1->SetBinError(5,4862.651);
   VbbHcc_tags_H_pt_stack_1->SetBinError(6,1429.655);
   VbbHcc_tags_H_pt_stack_1->SetBinError(7,1066.695);
   VbbHcc_tags_H_pt_stack_1->SetEntries(16749);

   ci = TColor::GetColor("#ff6600");
   VbbHcc_tags_H_pt_stack_1->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_tags_H_pt_stack_1->SetLineColor(ci);
   VbbHcc_tags_H_pt_stack_1->GetXaxis()->SetRange(1,2000);
   VbbHcc_tags_H_pt_stack_1->GetXaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_1->GetXaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_1->GetXaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_1->GetYaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_1->GetYaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_1->GetZaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_1->GetZaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_1->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_tags_H_pt_stack_1,"");
   
   TH1D *VbbHcc_tags_H_pt_stack_2 = new TH1D("VbbHcc_tags_H_pt_stack_2","",40,0,2000);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(1,48067.42);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(2,85159.42);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(3,47241.27);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(4,19213.09);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(5,8733.884);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(6,4425.011);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(7,2471.562);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(8,1434.188);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(9,850.2679);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(10,571.7755);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(11,357.3335);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(12,231.547);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(13,152.8724);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(14,102.1134);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(15,78.08197);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(16,51.4924);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(17,37.44613);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(18,24.91226);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(19,20.26032);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(20,13.02377);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(21,8.393989);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(22,9.492169);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(23,5.130737);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(24,4.399828);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(25,4.699847);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(26,4.518769);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(27,3.255294);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(28,1.77736);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(29,-0.006932703);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(30,1.207533);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(31,0.06368713);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(32,0.3359083);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(34,0.1738341);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(35,0.4052962);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(36,0.7330405);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(39,0.06310545);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(40,0.3973316);
   VbbHcc_tags_H_pt_stack_2->SetBinContent(41,0.2885821);
   VbbHcc_tags_H_pt_stack_2->SetBinError(1,93.31545);
   VbbHcc_tags_H_pt_stack_2->SetBinError(2,124.2139);
   VbbHcc_tags_H_pt_stack_2->SetBinError(3,94.51475);
   VbbHcc_tags_H_pt_stack_2->SetBinError(4,61.96158);
   VbbHcc_tags_H_pt_stack_2->SetBinError(5,42.77296);
   VbbHcc_tags_H_pt_stack_2->SetBinError(6,31.14024);
   VbbHcc_tags_H_pt_stack_2->SetBinError(7,23.74147);
   VbbHcc_tags_H_pt_stack_2->SetBinError(8,18.48704);
   VbbHcc_tags_H_pt_stack_2->SetBinError(9,14.42811);
   VbbHcc_tags_H_pt_stack_2->SetBinError(10,12.10083);
   VbbHcc_tags_H_pt_stack_2->SetBinError(11,9.63843);
   VbbHcc_tags_H_pt_stack_2->SetBinError(12,7.856107);
   VbbHcc_tags_H_pt_stack_2->SetBinError(13,6.38902);
   VbbHcc_tags_H_pt_stack_2->SetBinError(14,5.164551);
   VbbHcc_tags_H_pt_stack_2->SetBinError(15,4.780791);
   VbbHcc_tags_H_pt_stack_2->SetBinError(16,3.783865);
   VbbHcc_tags_H_pt_stack_2->SetBinError(17,3.216361);
   VbbHcc_tags_H_pt_stack_2->SetBinError(18,2.616766);
   VbbHcc_tags_H_pt_stack_2->SetBinError(19,2.361086);
   VbbHcc_tags_H_pt_stack_2->SetBinError(20,1.902278);
   VbbHcc_tags_H_pt_stack_2->SetBinError(21,1.548138);
   VbbHcc_tags_H_pt_stack_2->SetBinError(22,1.70937);
   VbbHcc_tags_H_pt_stack_2->SetBinError(23,1.37262);
   VbbHcc_tags_H_pt_stack_2->SetBinError(24,1.183498);
   VbbHcc_tags_H_pt_stack_2->SetBinError(25,1.202175);
   VbbHcc_tags_H_pt_stack_2->SetBinError(26,1.228631);
   VbbHcc_tags_H_pt_stack_2->SetBinError(27,1.044827);
   VbbHcc_tags_H_pt_stack_2->SetBinError(28,0.7055261);
   VbbHcc_tags_H_pt_stack_2->SetBinError(29,0.07576344);
   VbbHcc_tags_H_pt_stack_2->SetBinError(30,0.5443449);
   VbbHcc_tags_H_pt_stack_2->SetBinError(31,0.06368713);
   VbbHcc_tags_H_pt_stack_2->SetBinError(32,0.2956479);
   VbbHcc_tags_H_pt_stack_2->SetBinError(34,0.1010999);
   VbbHcc_tags_H_pt_stack_2->SetBinError(35,0.3521982);
   VbbHcc_tags_H_pt_stack_2->SetBinError(36,0.5204022);
   VbbHcc_tags_H_pt_stack_2->SetBinError(39,0.06310545);
   VbbHcc_tags_H_pt_stack_2->SetBinError(40,0.3973316);
   VbbHcc_tags_H_pt_stack_2->SetBinError(41,0.2451471);
   VbbHcc_tags_H_pt_stack_2->SetEntries(2852939);

   ci = TColor::GetColor("#660066");
   VbbHcc_tags_H_pt_stack_2->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_tags_H_pt_stack_2->SetLineColor(ci);
   VbbHcc_tags_H_pt_stack_2->GetXaxis()->SetRange(1,2000);
   VbbHcc_tags_H_pt_stack_2->GetXaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_2->GetXaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_2->GetXaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_2->GetYaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_2->GetYaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_2->GetZaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_2->GetZaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_2->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_tags_H_pt_stack_2,"");
   
   TH1D *VbbHcc_tags_H_pt_stack_3 = new TH1D("VbbHcc_tags_H_pt_stack_3","",40,0,2000);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(1,721523.5);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(2,1274300);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(3,705364.3);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(4,255537.2);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(5,104338.6);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(6,49367.62);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(7,25791.26);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(8,14309.16);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(9,8205.803);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(10,4882.645);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(11,3096.895);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(12,1963.338);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(13,1286.121);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(14,861.0855);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(15,577.5509);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(16,394.7658);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(17,277.6137);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(18,190.5569);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(19,133.5857);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(20,92.95226);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(21,71.77708);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(22,47.29489);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(23,36.80261);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(24,25.76508);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(25,19.47685);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(26,13.1642);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(27,8.725768);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(28,9.273224);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(29,6.244455);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(30,3.771429);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(31,3.689613);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(32,2.242216);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(33,1.429138);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(34,1.080047);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(35,1.148985);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(36,0.6424716);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(37,0.7359626);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(38,0.9982913);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(39,0.3546239);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(40,0.2077952);
   VbbHcc_tags_H_pt_stack_3->SetBinContent(41,0.6819511);
   VbbHcc_tags_H_pt_stack_3->SetBinError(1,231.8549);
   VbbHcc_tags_H_pt_stack_3->SetBinError(2,307.9948);
   VbbHcc_tags_H_pt_stack_3->SetBinError(3,229.1035);
   VbbHcc_tags_H_pt_stack_3->SetBinError(4,137.1124);
   VbbHcc_tags_H_pt_stack_3->SetBinError(5,87.3455);
   VbbHcc_tags_H_pt_stack_3->SetBinError(6,60.04488);
   VbbHcc_tags_H_pt_stack_3->SetBinError(7,43.45199);
   VbbHcc_tags_H_pt_stack_3->SetBinError(8,32.43682);
   VbbHcc_tags_H_pt_stack_3->SetBinError(9,24.62691);
   VbbHcc_tags_H_pt_stack_3->SetBinError(10,18.99509);
   VbbHcc_tags_H_pt_stack_3->SetBinError(11,15.21302);
   VbbHcc_tags_H_pt_stack_3->SetBinError(12,12.13232);
   VbbHcc_tags_H_pt_stack_3->SetBinError(13,9.835643);
   VbbHcc_tags_H_pt_stack_3->SetBinError(14,8.070386);
   VbbHcc_tags_H_pt_stack_3->SetBinError(15,6.625395);
   VbbHcc_tags_H_pt_stack_3->SetBinError(16,5.483194);
   VbbHcc_tags_H_pt_stack_3->SetBinError(17,4.600038);
   VbbHcc_tags_H_pt_stack_3->SetBinError(18,3.825542);
   VbbHcc_tags_H_pt_stack_3->SetBinError(19,3.200705);
   VbbHcc_tags_H_pt_stack_3->SetBinError(20,2.687478);
   VbbHcc_tags_H_pt_stack_3->SetBinError(21,2.340287);
   VbbHcc_tags_H_pt_stack_3->SetBinError(22,1.890229);
   VbbHcc_tags_H_pt_stack_3->SetBinError(23,1.704766);
   VbbHcc_tags_H_pt_stack_3->SetBinError(24,1.40813);
   VbbHcc_tags_H_pt_stack_3->SetBinError(25,1.217658);
   VbbHcc_tags_H_pt_stack_3->SetBinError(26,1.002336);
   VbbHcc_tags_H_pt_stack_3->SetBinError(27,0.8151072);
   VbbHcc_tags_H_pt_stack_3->SetBinError(28,0.859351);
   VbbHcc_tags_H_pt_stack_3->SetBinError(29,0.6944104);
   VbbHcc_tags_H_pt_stack_3->SetBinError(30,0.5566672);
   VbbHcc_tags_H_pt_stack_3->SetBinError(31,0.5448957);
   VbbHcc_tags_H_pt_stack_3->SetBinError(32,0.4206175);
   VbbHcc_tags_H_pt_stack_3->SetBinError(33,0.3462666);
   VbbHcc_tags_H_pt_stack_3->SetBinError(34,0.2975707);
   VbbHcc_tags_H_pt_stack_3->SetBinError(35,0.3164646);
   VbbHcc_tags_H_pt_stack_3->SetBinError(36,0.22029);
   VbbHcc_tags_H_pt_stack_3->SetBinError(37,0.2512061);
   VbbHcc_tags_H_pt_stack_3->SetBinError(38,0.2931405);
   VbbHcc_tags_H_pt_stack_3->SetBinError(39,0.1624554);
   VbbHcc_tags_H_pt_stack_3->SetBinError(40,0.1284453);
   VbbHcc_tags_H_pt_stack_3->SetBinError(41,0.2212994);
   VbbHcc_tags_H_pt_stack_3->SetEntries(4.734962e+07);

   ci = TColor::GetColor("#cc00cc");
   VbbHcc_tags_H_pt_stack_3->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_tags_H_pt_stack_3->SetLineColor(ci);
   VbbHcc_tags_H_pt_stack_3->GetXaxis()->SetRange(1,2000);
   VbbHcc_tags_H_pt_stack_3->GetXaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_3->GetXaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_3->GetXaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_3->GetYaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_3->GetYaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_3->GetZaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_3->GetZaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_3->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_tags_H_pt_stack_3,"");
   
   TH1D *VbbHcc_tags_H_pt_stack_4 = new TH1D("VbbHcc_tags_H_pt_stack_4","",40,0,2000);
   VbbHcc_tags_H_pt_stack_4->SetBinContent(1,6965.327);
   VbbHcc_tags_H_pt_stack_4->SetBinContent(2,14969.36);
   VbbHcc_tags_H_pt_stack_4->SetBinContent(3,14873.99);
   VbbHcc_tags_H_pt_stack_4->SetBinContent(4,10220.79);
   VbbHcc_tags_H_pt_stack_4->SetBinContent(5,4577.127);
   VbbHcc_tags_H_pt_stack_4->SetBinContent(6,1048.587);
   VbbHcc_tags_H_pt_stack_4->SetBinContent(7,90.08747);
   VbbHcc_tags_H_pt_stack_4->SetBinContent(8,3.650909);
   VbbHcc_tags_H_pt_stack_4->SetBinError(1,54.44948);
   VbbHcc_tags_H_pt_stack_4->SetBinError(2,79.81437);
   VbbHcc_tags_H_pt_stack_4->SetBinError(3,79.74451);
   VbbHcc_tags_H_pt_stack_4->SetBinError(4,66.04561);
   VbbHcc_tags_H_pt_stack_4->SetBinError(5,44.25818);
   VbbHcc_tags_H_pt_stack_4->SetBinError(6,21.16515);
   VbbHcc_tags_H_pt_stack_4->SetBinError(7,6.22197);
   VbbHcc_tags_H_pt_stack_4->SetBinError(8,1.235274);
   VbbHcc_tags_H_pt_stack_4->SetEntries(129710);

   ci = TColor::GetColor("#00cccc");
   VbbHcc_tags_H_pt_stack_4->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_tags_H_pt_stack_4->SetLineColor(ci);
   VbbHcc_tags_H_pt_stack_4->GetXaxis()->SetRange(1,2000);
   VbbHcc_tags_H_pt_stack_4->GetXaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_4->GetXaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_4->GetXaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_4->GetYaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_4->GetYaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_4->GetZaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_4->GetZaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_4->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_tags_H_pt_stack_4,"");
   
   TH1D *VbbHcc_tags_H_pt_stack_5 = new TH1D("VbbHcc_tags_H_pt_stack_5","",40,0,2000);
   VbbHcc_tags_H_pt_stack_5->SetBinContent(1,3928.844);
   VbbHcc_tags_H_pt_stack_5->SetBinContent(2,8932.096);
   VbbHcc_tags_H_pt_stack_5->SetBinContent(3,8989.632);
   VbbHcc_tags_H_pt_stack_5->SetBinContent(4,6527.375);
   VbbHcc_tags_H_pt_stack_5->SetBinContent(5,3239.909);
   VbbHcc_tags_H_pt_stack_5->SetBinContent(6,792.2812);
   VbbHcc_tags_H_pt_stack_5->SetBinContent(7,96.54536);
   VbbHcc_tags_H_pt_stack_5->SetBinContent(8,9.417782);
   VbbHcc_tags_H_pt_stack_5->SetBinContent(11,1.443374);
   VbbHcc_tags_H_pt_stack_5->SetBinError(1,72.11469);
   VbbHcc_tags_H_pt_stack_5->SetBinError(2,109.0084);
   VbbHcc_tags_H_pt_stack_5->SetBinError(3,109.1681);
   VbbHcc_tags_H_pt_stack_5->SetBinError(4,92.97454);
   VbbHcc_tags_H_pt_stack_5->SetBinError(5,65.63541);
   VbbHcc_tags_H_pt_stack_5->SetBinError(6,32.58568);
   VbbHcc_tags_H_pt_stack_5->SetBinError(7,11.41777);
   VbbHcc_tags_H_pt_stack_5->SetBinError(8,3.505106);
   VbbHcc_tags_H_pt_stack_5->SetBinError(11,1.443374);
   VbbHcc_tags_H_pt_stack_5->SetEntries(25836);

   ci = TColor::GetColor("#ff99cc");
   VbbHcc_tags_H_pt_stack_5->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_tags_H_pt_stack_5->SetLineColor(ci);
   VbbHcc_tags_H_pt_stack_5->GetXaxis()->SetRange(1,2000);
   VbbHcc_tags_H_pt_stack_5->GetXaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_5->GetXaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_5->GetXaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_5->GetYaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_5->GetYaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_5->GetZaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_5->GetZaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_5->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_tags_H_pt_stack_5,"");
   
   TH1D *VbbHcc_tags_H_pt_stack_6 = new TH1D("VbbHcc_tags_H_pt_stack_6","",40,0,2000);
   VbbHcc_tags_H_pt_stack_6->SetBinContent(1,275.7187);
   VbbHcc_tags_H_pt_stack_6->SetBinContent(2,382.8722);
   VbbHcc_tags_H_pt_stack_6->SetBinContent(3,191.9327);
   VbbHcc_tags_H_pt_stack_6->SetBinContent(4,92.01727);
   VbbHcc_tags_H_pt_stack_6->SetBinContent(5,46.01087);
   VbbHcc_tags_H_pt_stack_6->SetBinContent(6,24.14398);
   VbbHcc_tags_H_pt_stack_6->SetBinContent(7,13.85677);
   VbbHcc_tags_H_pt_stack_6->SetBinContent(8,9.140885);
   VbbHcc_tags_H_pt_stack_6->SetBinContent(9,4.504089);
   VbbHcc_tags_H_pt_stack_6->SetBinContent(10,3.589728);
   VbbHcc_tags_H_pt_stack_6->SetBinContent(11,2.025855);
   VbbHcc_tags_H_pt_stack_6->SetBinContent(12,2.547837);
   VbbHcc_tags_H_pt_stack_6->SetBinContent(13,1.504525);
   VbbHcc_tags_H_pt_stack_6->SetBinContent(14,1.024809);
   VbbHcc_tags_H_pt_stack_6->SetBinContent(15,0.7522708);
   VbbHcc_tags_H_pt_stack_6->SetBinContent(16,0.4195131);
   VbbHcc_tags_H_pt_stack_6->SetBinContent(17,0.8190312);
   VbbHcc_tags_H_pt_stack_6->SetBinContent(18,0.269307);
   VbbHcc_tags_H_pt_stack_6->SetBinContent(20,0.2552435);
   VbbHcc_tags_H_pt_stack_6->SetBinContent(23,0.2923392);
   VbbHcc_tags_H_pt_stack_6->SetBinError(1,8.611968);
   VbbHcc_tags_H_pt_stack_6->SetBinError(2,10.12891);
   VbbHcc_tags_H_pt_stack_6->SetBinError(3,7.153413);
   VbbHcc_tags_H_pt_stack_6->SetBinError(4,4.930777);
   VbbHcc_tags_H_pt_stack_6->SetBinError(5,3.443806);
   VbbHcc_tags_H_pt_stack_6->SetBinError(6,2.535368);
   VbbHcc_tags_H_pt_stack_6->SetBinError(7,1.956841);
   VbbHcc_tags_H_pt_stack_6->SetBinError(8,1.560132);
   VbbHcc_tags_H_pt_stack_6->SetBinError(9,1.095732);
   VbbHcc_tags_H_pt_stack_6->SetBinError(10,0.9261172);
   VbbHcc_tags_H_pt_stack_6->SetBinError(11,0.7338872);
   VbbHcc_tags_H_pt_stack_6->SetBinError(12,0.8554246);
   VbbHcc_tags_H_pt_stack_6->SetBinError(13,0.6237385);
   VbbHcc_tags_H_pt_stack_6->SetBinError(14,0.5151811);
   VbbHcc_tags_H_pt_stack_6->SetBinError(15,0.4374106);
   VbbHcc_tags_H_pt_stack_6->SetBinError(16,0.3016361);
   VbbHcc_tags_H_pt_stack_6->SetBinError(17,0.4730862);
   VbbHcc_tags_H_pt_stack_6->SetBinError(18,0.269307);
   VbbHcc_tags_H_pt_stack_6->SetBinError(20,0.2552435);
   VbbHcc_tags_H_pt_stack_6->SetBinError(23,0.2923392);
   VbbHcc_tags_H_pt_stack_6->SetEntries(4151);

   ci = TColor::GetColor("#9933ff");
   VbbHcc_tags_H_pt_stack_6->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_tags_H_pt_stack_6->SetLineColor(ci);
   VbbHcc_tags_H_pt_stack_6->GetXaxis()->SetRange(1,2000);
   VbbHcc_tags_H_pt_stack_6->GetXaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_6->GetXaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_6->GetXaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_6->GetYaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_6->GetYaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_6->GetZaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_6->GetZaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_6->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_tags_H_pt_stack_6,"");
   
   TH1D *VbbHcc_tags_H_pt_stack_7 = new TH1D("VbbHcc_tags_H_pt_stack_7","",40,0,2000);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(1,479.9759);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(2,689.4803);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(3,346.0206);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(4,178.3636);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(5,93.04855);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(6,54.3691);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(7,32.10099);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(8,17.25438);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(9,9.54834);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(10,6.70495);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(11,4.751315);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(12,1.311252);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(13,0.7768221);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(14,2.018039);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(15,0.5267701);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(16,1.033765);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(17,0.2487814);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(19,0.365159);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(20,0.3514358);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(21,0.1801457);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(22,0.1236251);
   VbbHcc_tags_H_pt_stack_7->SetBinContent(25,0.1336818);
   VbbHcc_tags_H_pt_stack_7->SetBinError(1,9.041484);
   VbbHcc_tags_H_pt_stack_7->SetBinError(2,10.78385);
   VbbHcc_tags_H_pt_stack_7->SetBinError(3,7.620215);
   VbbHcc_tags_H_pt_stack_7->SetBinError(4,5.473055);
   VbbHcc_tags_H_pt_stack_7->SetBinError(5,3.94054);
   VbbHcc_tags_H_pt_stack_7->SetBinError(6,3.031873);
   VbbHcc_tags_H_pt_stack_7->SetBinError(7,2.349431);
   VbbHcc_tags_H_pt_stack_7->SetBinError(8,1.702926);
   VbbHcc_tags_H_pt_stack_7->SetBinError(9,1.242831);
   VbbHcc_tags_H_pt_stack_7->SetBinError(10,1.038373);
   VbbHcc_tags_H_pt_stack_7->SetBinError(11,0.8960742);
   VbbHcc_tags_H_pt_stack_7->SetBinError(12,0.4697417);
   VbbHcc_tags_H_pt_stack_7->SetBinError(13,0.3599854);
   VbbHcc_tags_H_pt_stack_7->SetBinError(14,0.5869865);
   VbbHcc_tags_H_pt_stack_7->SetBinError(15,0.3055384);
   VbbHcc_tags_H_pt_stack_7->SetBinError(16,0.4328804);
   VbbHcc_tags_H_pt_stack_7->SetBinError(17,0.1759153);
   VbbHcc_tags_H_pt_stack_7->SetBinError(19,0.2582211);
   VbbHcc_tags_H_pt_stack_7->SetBinError(20,0.2487649);
   VbbHcc_tags_H_pt_stack_7->SetBinError(21,0.1801457);
   VbbHcc_tags_H_pt_stack_7->SetBinError(22,0.1236251);
   VbbHcc_tags_H_pt_stack_7->SetBinError(25,0.1336818);
   VbbHcc_tags_H_pt_stack_7->SetEntries(11906);

   ci = TColor::GetColor("#3399ff");
   VbbHcc_tags_H_pt_stack_7->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_tags_H_pt_stack_7->SetLineColor(ci);
   VbbHcc_tags_H_pt_stack_7->GetXaxis()->SetRange(1,2000);
   VbbHcc_tags_H_pt_stack_7->GetXaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_7->GetXaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_7->GetXaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_7->GetYaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_7->GetYaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_7->GetZaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_7->GetZaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_7->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_tags_H_pt_stack_7,"");
   
   TH1D *VbbHcc_tags_H_pt_stack_8 = new TH1D("VbbHcc_tags_H_pt_stack_8","",40,0,2000);
   VbbHcc_tags_H_pt_stack_8->SetBinContent(1,498.163);
   VbbHcc_tags_H_pt_stack_8->SetBinContent(2,649.0093);
   VbbHcc_tags_H_pt_stack_8->SetBinContent(3,286.993);
   VbbHcc_tags_H_pt_stack_8->SetBinContent(4,146.2695);
   VbbHcc_tags_H_pt_stack_8->SetBinContent(5,75.86423);
   VbbHcc_tags_H_pt_stack_8->SetBinContent(6,44.82062);
   VbbHcc_tags_H_pt_stack_8->SetBinContent(7,22.27114);
   VbbHcc_tags_H_pt_stack_8->SetBinContent(8,9.813731);
   VbbHcc_tags_H_pt_stack_8->SetBinContent(9,10.05844);
   VbbHcc_tags_H_pt_stack_8->SetBinContent(10,4.142232);
   VbbHcc_tags_H_pt_stack_8->SetBinContent(11,3.153529);
   VbbHcc_tags_H_pt_stack_8->SetBinContent(12,0.5032325);
   VbbHcc_tags_H_pt_stack_8->SetBinContent(13,1.432682);
   VbbHcc_tags_H_pt_stack_8->SetBinContent(14,0.4996567);
   VbbHcc_tags_H_pt_stack_8->SetBinContent(18,0.5729236);
   VbbHcc_tags_H_pt_stack_8->SetBinContent(22,0.4945403);
   VbbHcc_tags_H_pt_stack_8->SetBinError(1,15.27086);
   VbbHcc_tags_H_pt_stack_8->SetBinError(2,17.2747);
   VbbHcc_tags_H_pt_stack_8->SetBinError(3,11.4701);
   VbbHcc_tags_H_pt_stack_8->SetBinError(4,8.162133);
   VbbHcc_tags_H_pt_stack_8->SetBinError(5,5.94771);
   VbbHcc_tags_H_pt_stack_8->SetBinError(6,4.52868);
   VbbHcc_tags_H_pt_stack_8->SetBinError(7,3.193145);
   VbbHcc_tags_H_pt_stack_8->SetBinError(8,2.047434);
   VbbHcc_tags_H_pt_stack_8->SetBinError(9,2.08025);
   VbbHcc_tags_H_pt_stack_8->SetBinError(10,1.395452);
   VbbHcc_tags_H_pt_stack_8->SetBinError(11,1.291087);
   VbbHcc_tags_H_pt_stack_8->SetBinError(12,0.5032325);
   VbbHcc_tags_H_pt_stack_8->SetBinError(13,0.8413678);
   VbbHcc_tags_H_pt_stack_8->SetBinError(14,0.4996567);
   VbbHcc_tags_H_pt_stack_8->SetBinError(18,0.5729236);
   VbbHcc_tags_H_pt_stack_8->SetBinError(22,0.4945403);
   VbbHcc_tags_H_pt_stack_8->SetEntries(3978);

   ci = TColor::GetColor("#33ff99");
   VbbHcc_tags_H_pt_stack_8->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_tags_H_pt_stack_8->SetLineColor(ci);
   VbbHcc_tags_H_pt_stack_8->GetXaxis()->SetRange(1,2000);
   VbbHcc_tags_H_pt_stack_8->GetXaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_8->GetXaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_8->GetXaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_8->GetYaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_8->GetYaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_8->GetZaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_8->GetZaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_8->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_tags_H_pt_stack_8,"");
   
   TH1D *VbbHcc_tags_H_pt_stack_9 = new TH1D("VbbHcc_tags_H_pt_stack_9","",40,0,2000);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(1,130.8543);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(2,210.8468);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(3,120.2713);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(4,58.66706);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(5,31.3595);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(6,17.16227);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(7,10.07163);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(8,6.085838);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(9,3.515664);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(10,1.837629);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(11,1.131312);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(12,0.6316509);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(13,0.389279);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(14,0.2857061);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(15,0.1470487);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(16,0.117575);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(17,0.07782114);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(18,0.0511972);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(19,0.04187873);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(20,0.02787685);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(21,0.01914062);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(22,0.01372779);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(23,0.01181042);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(24,0.003364558);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(25,0.001953322);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(26,0.008927842);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(27,0.001746513);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(28,0.002068506);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(30,0.004340334);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(32,0.001992314);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(33,0.001676022);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(34,0.001911414);
   VbbHcc_tags_H_pt_stack_9->SetBinContent(36,0.001772933);
   VbbHcc_tags_H_pt_stack_9->SetBinError(1,0.5040475);
   VbbHcc_tags_H_pt_stack_9->SetBinError(2,0.6385153);
   VbbHcc_tags_H_pt_stack_9->SetBinError(3,0.4806325);
   VbbHcc_tags_H_pt_stack_9->SetBinError(4,0.3357275);
   VbbHcc_tags_H_pt_stack_9->SetBinError(5,0.2457808);
   VbbHcc_tags_H_pt_stack_9->SetBinError(6,0.1816965);
   VbbHcc_tags_H_pt_stack_9->SetBinError(7,0.1387778);
   VbbHcc_tags_H_pt_stack_9->SetBinError(8,0.1083078);
   VbbHcc_tags_H_pt_stack_9->SetBinError(9,0.08204049);
   VbbHcc_tags_H_pt_stack_9->SetBinError(10,0.05911666);
   VbbHcc_tags_H_pt_stack_9->SetBinError(11,0.04597126);
   VbbHcc_tags_H_pt_stack_9->SetBinError(12,0.0342664);
   VbbHcc_tags_H_pt_stack_9->SetBinError(13,0.02698049);
   VbbHcc_tags_H_pt_stack_9->SetBinError(14,0.02301204);
   VbbHcc_tags_H_pt_stack_9->SetBinError(15,0.01620228);
   VbbHcc_tags_H_pt_stack_9->SetBinError(16,0.01485274);
   VbbHcc_tags_H_pt_stack_9->SetBinError(17,0.01197626);
   VbbHcc_tags_H_pt_stack_9->SetBinError(18,0.009710055);
   VbbHcc_tags_H_pt_stack_9->SetBinError(19,0.008855692);
   VbbHcc_tags_H_pt_stack_9->SetBinError(20,0.007511492);
   VbbHcc_tags_H_pt_stack_9->SetBinError(21,0.006077229);
   VbbHcc_tags_H_pt_stack_9->SetBinError(22,0.005255502);
   VbbHcc_tags_H_pt_stack_9->SetBinError(23,0.004555788);
   VbbHcc_tags_H_pt_stack_9->SetBinError(24,0.002414627);
   VbbHcc_tags_H_pt_stack_9->SetBinError(25,0.001953322);
   VbbHcc_tags_H_pt_stack_9->SetBinError(26,0.004047487);
   VbbHcc_tags_H_pt_stack_9->SetBinError(27,0.001746513);
   VbbHcc_tags_H_pt_stack_9->SetBinError(28,0.002068506);
   VbbHcc_tags_H_pt_stack_9->SetBinError(30,0.003098179);
   VbbHcc_tags_H_pt_stack_9->SetBinError(32,0.001992314);
   VbbHcc_tags_H_pt_stack_9->SetBinError(33,0.001676022);
   VbbHcc_tags_H_pt_stack_9->SetBinError(34,0.001911414);
   VbbHcc_tags_H_pt_stack_9->SetBinError(36,0.001772933);
   VbbHcc_tags_H_pt_stack_9->SetEntries(337563);

   ci = TColor::GetColor("#cccc00");
   VbbHcc_tags_H_pt_stack_9->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_tags_H_pt_stack_9->SetLineColor(ci);
   VbbHcc_tags_H_pt_stack_9->GetXaxis()->SetRange(1,2000);
   VbbHcc_tags_H_pt_stack_9->GetXaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_9->GetXaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_9->GetXaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_9->GetYaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_9->GetYaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_9->GetZaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_9->GetZaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_9->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_tags_H_pt_stack_9,"");
   
   TH1D *VbbHcc_tags_H_pt_stack_10 = new TH1D("VbbHcc_tags_H_pt_stack_10","",40,0,2000);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(1,20.30813);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(2,42.65982);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(3,38.09881);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(4,23.83931);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(5,13.28502);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(6,7.530712);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(7,4.383724);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(8,2.771671);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(9,1.687693);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(10,1.083586);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(11,0.7034269);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(12,0.4966946);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(13,0.3407227);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(14,0.2469483);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(15,0.1753977);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(16,0.1180989);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(17,0.09406348);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(18,0.07027216);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(19,0.05513899);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(20,0.03858432);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(21,0.02465868);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(22,0.02359522);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(23,0.01458466);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(24,0.01609578);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(25,0.01154052);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(26,0.01092484);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(27,0.005022946);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(28,0.006073635);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(29,0.004205853);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(30,0.004419335);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(31,0.001818593);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(32,0.001812742);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(33,0.002246452);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(34,0.001324114);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(35,0.0008856872);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(37,0.000449816);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(38,0.000423518);
   VbbHcc_tags_H_pt_stack_10->SetBinContent(41,0.001313887);
   VbbHcc_tags_H_pt_stack_10->SetBinError(1,0.09495873);
   VbbHcc_tags_H_pt_stack_10->SetBinError(2,0.1376095);
   VbbHcc_tags_H_pt_stack_10->SetBinError(3,0.1300306);
   VbbHcc_tags_H_pt_stack_10->SetBinError(4,0.1027982);
   VbbHcc_tags_H_pt_stack_10->SetBinError(5,0.07666467);
   VbbHcc_tags_H_pt_stack_10->SetBinError(6,0.0576825);
   VbbHcc_tags_H_pt_stack_10->SetBinError(7,0.0440112);
   VbbHcc_tags_H_pt_stack_10->SetBinError(8,0.03497405);
   VbbHcc_tags_H_pt_stack_10->SetBinError(9,0.02728532);
   VbbHcc_tags_H_pt_stack_10->SetBinError(10,0.02186604);
   VbbHcc_tags_H_pt_stack_10->SetBinError(11,0.0175971);
   VbbHcc_tags_H_pt_stack_10->SetBinError(12,0.01484582);
   VbbHcc_tags_H_pt_stack_10->SetBinError(13,0.01229517);
   VbbHcc_tags_H_pt_stack_10->SetBinError(14,0.01048535);
   VbbHcc_tags_H_pt_stack_10->SetBinError(15,0.008804616);
   VbbHcc_tags_H_pt_stack_10->SetBinError(16,0.007242831);
   VbbHcc_tags_H_pt_stack_10->SetBinError(17,0.006450806);
   VbbHcc_tags_H_pt_stack_10->SetBinError(18,0.005562792);
   VbbHcc_tags_H_pt_stack_10->SetBinError(19,0.004945555);
   VbbHcc_tags_H_pt_stack_10->SetBinError(20,0.004163285);
   VbbHcc_tags_H_pt_stack_10->SetBinError(21,0.003312125);
   VbbHcc_tags_H_pt_stack_10->SetBinError(22,0.003233221);
   VbbHcc_tags_H_pt_stack_10->SetBinError(23,0.002541721);
   VbbHcc_tags_H_pt_stack_10->SetBinError(24,0.002677255);
   VbbHcc_tags_H_pt_stack_10->SetBinError(25,0.002254187);
   VbbHcc_tags_H_pt_stack_10->SetBinError(26,0.002195664);
   VbbHcc_tags_H_pt_stack_10->SetBinError(27,0.001473103);
   VbbHcc_tags_H_pt_stack_10->SetBinError(28,0.001687784);
   VbbHcc_tags_H_pt_stack_10->SetBinError(29,0.001347866);
   VbbHcc_tags_H_pt_stack_10->SetBinError(30,0.001398067);
   VbbHcc_tags_H_pt_stack_10->SetBinError(31,0.000909516);
   VbbHcc_tags_H_pt_stack_10->SetBinError(32,0.0009067979);
   VbbHcc_tags_H_pt_stack_10->SetBinError(33,0.001006291);
   VbbHcc_tags_H_pt_stack_10->SetBinError(34,0.0007646366);
   VbbHcc_tags_H_pt_stack_10->SetBinError(35,0.000626355);
   VbbHcc_tags_H_pt_stack_10->SetBinError(37,0.000449816);
   VbbHcc_tags_H_pt_stack_10->SetBinError(38,0.000423518);
   VbbHcc_tags_H_pt_stack_10->SetBinError(41,0.000758588);
   VbbHcc_tags_H_pt_stack_10->SetEntries(363165);

   ci = TColor::GetColor("#0000cc");
   VbbHcc_tags_H_pt_stack_10->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_tags_H_pt_stack_10->SetLineColor(ci);
   VbbHcc_tags_H_pt_stack_10->GetXaxis()->SetRange(1,2000);
   VbbHcc_tags_H_pt_stack_10->GetXaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_10->GetXaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_10->GetXaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_10->GetYaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_10->GetYaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_10->GetZaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_10->GetZaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_10->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_tags_H_pt_stack_10,"");
   
   TH1D *VbbHcc_tags_H_pt_stack_11 = new TH1D("VbbHcc_tags_H_pt_stack_11","",40,0,2000);
   VbbHcc_tags_H_pt_stack_11->SetBinContent(1,2.106539);
   VbbHcc_tags_H_pt_stack_11->SetBinContent(2,3.396448);
   VbbHcc_tags_H_pt_stack_11->SetBinContent(3,2.046789);
   VbbHcc_tags_H_pt_stack_11->SetBinContent(4,1.076955);
   VbbHcc_tags_H_pt_stack_11->SetBinContent(5,0.4348517);
   VbbHcc_tags_H_pt_stack_11->SetBinContent(6,0.2247412);
   VbbHcc_tags_H_pt_stack_11->SetBinContent(7,0.1133214);
   VbbHcc_tags_H_pt_stack_11->SetBinContent(8,0.08199119);
   VbbHcc_tags_H_pt_stack_11->SetBinContent(9,0.05862219);
   VbbHcc_tags_H_pt_stack_11->SetBinContent(10,0.01475829);
   VbbHcc_tags_H_pt_stack_11->SetBinContent(11,0.01332778);
   VbbHcc_tags_H_pt_stack_11->SetBinContent(12,0.005634108);
   VbbHcc_tags_H_pt_stack_11->SetBinContent(14,0.003007753);
   VbbHcc_tags_H_pt_stack_11->SetBinContent(15,0.001259297);
   VbbHcc_tags_H_pt_stack_11->SetBinContent(16,0.001727301);
   VbbHcc_tags_H_pt_stack_11->SetBinError(1,0.06257864);
   VbbHcc_tags_H_pt_stack_11->SetBinError(2,0.07883449);
   VbbHcc_tags_H_pt_stack_11->SetBinError(3,0.0611923);
   VbbHcc_tags_H_pt_stack_11->SetBinError(4,0.04413958);
   VbbHcc_tags_H_pt_stack_11->SetBinError(5,0.02773564);
   VbbHcc_tags_H_pt_stack_11->SetBinError(6,0.0205148);
   VbbHcc_tags_H_pt_stack_11->SetBinError(7,0.0144079);
   VbbHcc_tags_H_pt_stack_11->SetBinError(8,0.01221683);
   VbbHcc_tags_H_pt_stack_11->SetBinError(9,0.01042476);
   VbbHcc_tags_H_pt_stack_11->SetBinError(10,0.005049608);
   VbbHcc_tags_H_pt_stack_11->SetBinError(11,0.00471908);
   VbbHcc_tags_H_pt_stack_11->SetBinError(12,0.002991408);
   VbbHcc_tags_H_pt_stack_11->SetBinError(14,0.00212737);
   VbbHcc_tags_H_pt_stack_11->SetBinError(15,0.001259297);
   VbbHcc_tags_H_pt_stack_11->SetBinError(16,0.001727301);
   VbbHcc_tags_H_pt_stack_11->SetEntries(6396);

   ci = TColor::GetColor("#cc0000");
   VbbHcc_tags_H_pt_stack_11->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_tags_H_pt_stack_11->SetLineColor(ci);
   VbbHcc_tags_H_pt_stack_11->GetXaxis()->SetRange(1,2000);
   VbbHcc_tags_H_pt_stack_11->GetXaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_11->GetXaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_11->GetXaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_11->GetYaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_11->GetYaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_11->GetZaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_11->GetZaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_11->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_tags_H_pt_stack_11,"");
   
   TH1D *VbbHcc_tags_H_pt_stack_12 = new TH1D("VbbHcc_tags_H_pt_stack_12","",40,0,2000);
   VbbHcc_tags_H_pt_stack_12->SetBinContent(1,6534.672);
   VbbHcc_tags_H_pt_stack_12->SetBinContent(2,14858.46);
   VbbHcc_tags_H_pt_stack_12->SetBinContent(3,15899.08);
   VbbHcc_tags_H_pt_stack_12->SetBinContent(4,10151.45);
   VbbHcc_tags_H_pt_stack_12->SetBinContent(5,5076.104);
   VbbHcc_tags_H_pt_stack_12->SetBinContent(6,2245.424);
   VbbHcc_tags_H_pt_stack_12->SetBinContent(7,1285.017);
   VbbHcc_tags_H_pt_stack_12->SetBinContent(8,619.0327);
   VbbHcc_tags_H_pt_stack_12->SetBinContent(9,269.3397);
   VbbHcc_tags_H_pt_stack_12->SetBinContent(10,121.4886);
   VbbHcc_tags_H_pt_stack_12->SetBinContent(11,46.02068);
   VbbHcc_tags_H_pt_stack_12->SetBinContent(12,44.12563);
   VbbHcc_tags_H_pt_stack_12->SetBinContent(13,22.25861);
   VbbHcc_tags_H_pt_stack_12->SetBinContent(14,15.52406);
   VbbHcc_tags_H_pt_stack_12->SetBinContent(15,6.127426);
   VbbHcc_tags_H_pt_stack_12->SetBinContent(16,4.878256);
   VbbHcc_tags_H_pt_stack_12->SetBinContent(23,3.117953);
   VbbHcc_tags_H_pt_stack_12->SetBinError(1,184.6668);
   VbbHcc_tags_H_pt_stack_12->SetBinError(2,279.0524);
   VbbHcc_tags_H_pt_stack_12->SetBinError(3,288.1521);
   VbbHcc_tags_H_pt_stack_12->SetBinError(4,230.0621);
   VbbHcc_tags_H_pt_stack_12->SetBinError(5,161.9102);
   VbbHcc_tags_H_pt_stack_12->SetBinError(6,108.3397);
   VbbHcc_tags_H_pt_stack_12->SetBinError(7,80.31584);
   VbbHcc_tags_H_pt_stack_12->SetBinError(8,56.92859);
   VbbHcc_tags_H_pt_stack_12->SetBinError(9,36.47871);
   VbbHcc_tags_H_pt_stack_12->SetBinError(10,25.36625);
   VbbHcc_tags_H_pt_stack_12->SetBinError(11,14.66324);
   VbbHcc_tags_H_pt_stack_12->SetBinError(12,15.26325);
   VbbHcc_tags_H_pt_stack_12->SetBinError(13,11.08125);
   VbbHcc_tags_H_pt_stack_12->SetBinError(14,9.044982);
   VbbHcc_tags_H_pt_stack_12->SetBinError(15,4.820284);
   VbbHcc_tags_H_pt_stack_12->SetBinError(16,4.878256);
   VbbHcc_tags_H_pt_stack_12->SetBinError(23,3.117953);
   VbbHcc_tags_H_pt_stack_12->SetEntries(12638);

   ci = TColor::GetColor("#00cc00");
   VbbHcc_tags_H_pt_stack_12->SetFillColor(ci);

   ci = TColor::GetColor("#000099");
   VbbHcc_tags_H_pt_stack_12->SetLineColor(ci);
   VbbHcc_tags_H_pt_stack_12->GetXaxis()->SetRange(1,2000);
   VbbHcc_tags_H_pt_stack_12->GetXaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_12->GetXaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_12->GetXaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_12->GetYaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_12->GetYaxis()->SetTitleFont(42);
   VbbHcc_tags_H_pt_stack_12->GetZaxis()->SetLabelFont(42);
   VbbHcc_tags_H_pt_stack_12->GetZaxis()->SetTitleOffset(1);
   VbbHcc_tags_H_pt_stack_12->GetZaxis()->SetTitleFont(42);
   st->Add(VbbHcc_tags_H_pt_stack_12,"");
   st->Draw("hist");
   
   TLegend *leg = new TLegend(0.48,0.62,0.85,0.87,NULL,"brNDC");
   leg->SetBorderSize(0);
   leg->SetTextSize(0.035);
   leg->SetLineColor(1);
   leg->SetLineStyle(1);
   leg->SetLineWidth(2);
   leg->SetFillColor(0);
   leg->SetFillStyle(1001);
   TLegendEntry *entry=leg->AddEntry("VbbHcc_tags_H_pt_stack_12","ggZHcc","F");

   ci = TColor::GetColor("#00cc00");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_tags_H_pt_stack_11","ZHcc","F");

   ci = TColor::GetColor("#cc0000");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_tags_H_pt_stack_10","ggZHbb","F");

   ci = TColor::GetColor("#0000cc");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_tags_H_pt_stack_9","ZHbb","F");

   ci = TColor::GetColor("#cccc00");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_tags_H_pt_stack_8","ZZ","F");

   ci = TColor::GetColor("#33ff99");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_tags_H_pt_stack_7","WZ","F");

   ci = TColor::GetColor("#3399ff");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_tags_H_pt_stack_6","WW","F");

   ci = TColor::GetColor("#9933ff");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_tags_H_pt_stack_5","W + jets","F");

   ci = TColor::GetColor("#ff99cc");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_tags_H_pt_stack_4","Z + jets","F");

   ci = TColor::GetColor("#00cccc");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_tags_H_pt_stack_3","t#bar{t}","F");

   ci = TColor::GetColor("#cc00cc");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_tags_H_pt_stack_2","Single Top","F");

   ci = TColor::GetColor("#660066");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("VbbHcc_tags_H_pt_stack_1","QCD","F");

   ci = TColor::GetColor("#ff6600");
   entry->SetFillColor(ci);
   entry->SetFillStyle(1001);

   ci = TColor::GetColor("#000099");
   entry->SetLineColor(ci);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   entry->SetMarkerColor(1);
   entry->SetMarkerStyle(21);
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   leg->Draw();
   TLatex *   tex = new TLatex(0.25,0.937775,"CMS Work in Progress #sqrt{s} = 13 TeV, 41.5 fb^{-1}");
tex->SetNDC();
   tex->SetTextFont(42);
   tex->SetTextSize(0.04);
   tex->SetLineWidth(2);
   tex->Draw();
   H_pt_tags_17->Modified();
   H_pt_tags_17->cd();
   H_pt_tags_17->SetSelected(H_pt_tags_17);
}
