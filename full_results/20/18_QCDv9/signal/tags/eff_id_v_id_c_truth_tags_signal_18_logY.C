#ifdef __CLING__
#pragma cling optimize(0)
#endif
void eff_id_v_id_c_truth_tags_signal_18_logY()
{
//=========Macro generated from canvas: eff_id_v_id_c_truth_tags_signal_18/eff_id_v_id_c_truth_tags_signal_18
//=========  (Thu Mar  2 13:37:21 2023) by ROOT version 6.26/06
   TCanvas *eff_id_v_id_c_truth_tags_signal_18 = new TCanvas("eff_id_v_id_c_truth_tags_signal_18", "eff_id_v_id_c_truth_tags_signal_18",0,0,600,600);
   gStyle->SetOptStat(0);
   eff_id_v_id_c_truth_tags_signal_18->SetHighLightColor(2);
   eff_id_v_id_c_truth_tags_signal_18->Range(-0.8,-0.0752575,4.533333,0.6773175);
   eff_id_v_id_c_truth_tags_signal_18->SetFillColor(0);
   eff_id_v_id_c_truth_tags_signal_18->SetBorderMode(0);
   eff_id_v_id_c_truth_tags_signal_18->SetBorderSize(2);
   eff_id_v_id_c_truth_tags_signal_18->SetLogy();
   eff_id_v_id_c_truth_tags_signal_18->SetLeftMargin(0.15);
   eff_id_v_id_c_truth_tags_signal_18->SetFrameBorderMode(0);
   eff_id_v_id_c_truth_tags_signal_18->SetFrameBorderMode(0);
   
   TH2D *VbbHcc_eff_tags_id_v_id_c_truth = new TH2D("VbbHcc_eff_tags_id_v_id_c_truth","",4,0,4,4,0,4);
   VbbHcc_eff_tags_id_v_id_c_truth->SetBinContent(7,1.058963);
   VbbHcc_eff_tags_id_v_id_c_truth->SetBinContent(8,3.438734);
   VbbHcc_eff_tags_id_v_id_c_truth->SetBinContent(13,3.30824);
   VbbHcc_eff_tags_id_v_id_c_truth->SetBinContent(14,1.01745);
   VbbHcc_eff_tags_id_v_id_c_truth->SetBinError(7,0.05331983);
   VbbHcc_eff_tags_id_v_id_c_truth->SetBinError(8,0.09468056);
   VbbHcc_eff_tags_id_v_id_c_truth->SetBinError(13,0.08482714);
   VbbHcc_eff_tags_id_v_id_c_truth->SetBinError(14,0.04925764);
   VbbHcc_eff_tags_id_v_id_c_truth->SetEntries(10434);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContour(20);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(0,0);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(1,0.1719367);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(2,0.3438734);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(3,0.5158101);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(4,0.6877467);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(5,0.8596834);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(6,1.03162);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(7,1.203557);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(8,1.375493);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(9,1.54743);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(10,1.719367);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(11,1.891304);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(12,2.06324);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(13,2.235177);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(14,2.407114);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(15,2.57905);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(16,2.750987);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(17,2.922924);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(18,3.09486);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(19,3.266797);
   
   TPaletteAxis *palette = new TPaletteAxis(4.026667,1.387779e-17,4.266667,0.60206,VbbHcc_eff_tags_id_v_id_c_truth);
   palette->SetLabelColor(1);
   palette->SetLabelFont(42);
   palette->SetLabelOffset(0.005);
   palette->SetLabelSize(0.035);
   palette->SetTitleOffset(1);
   palette->SetTitleSize(0.035);

   Int_t ci;      // for color index setting
   TColor *color; // for color definition with alpha
   ci = TColor::GetColor("#f9f90e");
   palette->SetFillColor(ci);
   palette->SetFillStyle(1001);
   VbbHcc_eff_tags_id_v_id_c_truth->GetListOfFunctions()->Add(palette,"br");

   ci = TColor::GetColor("#ff0000");
   VbbHcc_eff_tags_id_v_id_c_truth->SetLineColor(ci);
   VbbHcc_eff_tags_id_v_id_c_truth->GetXaxis()->SetTitle("index (c_{0})");
   VbbHcc_eff_tags_id_v_id_c_truth->GetXaxis()->SetLabelFont(42);
   VbbHcc_eff_tags_id_v_id_c_truth->GetXaxis()->SetTitleOffset(1);
   VbbHcc_eff_tags_id_v_id_c_truth->GetXaxis()->SetTitleFont(42);
   VbbHcc_eff_tags_id_v_id_c_truth->GetYaxis()->SetTitle("index (c_{1})");
   VbbHcc_eff_tags_id_v_id_c_truth->GetYaxis()->SetLabelFont(42);
   VbbHcc_eff_tags_id_v_id_c_truth->GetYaxis()->SetTitleFont(42);
   VbbHcc_eff_tags_id_v_id_c_truth->GetZaxis()->SetLabelFont(42);
   VbbHcc_eff_tags_id_v_id_c_truth->GetZaxis()->SetTitleOffset(1);
   VbbHcc_eff_tags_id_v_id_c_truth->GetZaxis()->SetTitleFont(42);
   VbbHcc_eff_tags_id_v_id_c_truth->Draw("colz");
   eff_id_v_id_c_truth_tags_signal_18->Modified();
   eff_id_v_id_c_truth_tags_signal_18->cd();
   eff_id_v_id_c_truth_tags_signal_18->SetSelected(eff_id_v_id_c_truth_tags_signal_18);
}
