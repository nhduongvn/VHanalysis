#ifdef __CLING__
#pragma cling optimize(0)
#endif
void eff_id_v_id_c_truth_tags_signal_16_logY()
{
//=========Macro generated from canvas: eff_id_v_id_c_truth_tags_signal_16/eff_id_v_id_c_truth_tags_signal_16
//=========  (Thu Mar  2 13:37:21 2023) by ROOT version 6.26/06
   TCanvas *eff_id_v_id_c_truth_tags_signal_16 = new TCanvas("eff_id_v_id_c_truth_tags_signal_16", "eff_id_v_id_c_truth_tags_signal_16",0,0,600,600);
   gStyle->SetOptStat(0);
   eff_id_v_id_c_truth_tags_signal_16->SetHighLightColor(2);
   eff_id_v_id_c_truth_tags_signal_16->Range(-0.8,-0.0752575,4.533333,0.6773175);
   eff_id_v_id_c_truth_tags_signal_16->SetFillColor(0);
   eff_id_v_id_c_truth_tags_signal_16->SetBorderMode(0);
   eff_id_v_id_c_truth_tags_signal_16->SetBorderSize(2);
   eff_id_v_id_c_truth_tags_signal_16->SetLogy();
   eff_id_v_id_c_truth_tags_signal_16->SetLeftMargin(0.15);
   eff_id_v_id_c_truth_tags_signal_16->SetFrameBorderMode(0);
   eff_id_v_id_c_truth_tags_signal_16->SetFrameBorderMode(0);
   
   TH2D *VbbHcc_eff_tags_id_v_id_c_truth = new TH2D("VbbHcc_eff_tags_id_v_id_c_truth","",4,0,4,4,0,4);
   VbbHcc_eff_tags_id_v_id_c_truth->SetBinContent(7,0.5300985);
   VbbHcc_eff_tags_id_v_id_c_truth->SetBinContent(8,1.586736);
   VbbHcc_eff_tags_id_v_id_c_truth->SetBinContent(13,1.607321);
   VbbHcc_eff_tags_id_v_id_c_truth->SetBinContent(14,0.4929007);
   VbbHcc_eff_tags_id_v_id_c_truth->SetBinError(7,0.03412543);
   VbbHcc_eff_tags_id_v_id_c_truth->SetBinError(8,0.05980032);
   VbbHcc_eff_tags_id_v_id_c_truth->SetBinError(13,0.05901768);
   VbbHcc_eff_tags_id_v_id_c_truth->SetBinError(14,0.03198586);
   VbbHcc_eff_tags_id_v_id_c_truth->SetEntries(4443);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContour(20);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(0,0);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(1,0.08036606);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(2,0.1607321);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(3,0.2410982);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(4,0.3214642);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(5,0.4018303);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(6,0.4821963);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(7,0.5625624);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(8,0.6429284);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(9,0.7232945);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(10,0.8036606);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(11,0.8840266);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(12,0.9643927);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(13,1.044759);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(14,1.125125);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(15,1.205491);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(16,1.285857);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(17,1.366223);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(18,1.446589);
   VbbHcc_eff_tags_id_v_id_c_truth->SetContourLevel(19,1.526955);
   
   TPaletteAxis *palette = new TPaletteAxis(4.026667,1.387779e-17,4.266667,0.60206,VbbHcc_eff_tags_id_v_id_c_truth);
   palette->SetLabelColor(1);
   palette->SetLabelFont(42);
   palette->SetLabelOffset(0.005);
   palette->SetLabelSize(0.035);
   palette->SetTitleOffset(1);
   palette->SetTitleSize(0.035);

   Int_t ci;      // for color index setting
   TColor *color; // for color definition with alpha
   ci = TColor::GetColor("#f9f90e");
   palette->SetFillColor(ci);
   palette->SetFillStyle(1001);
   VbbHcc_eff_tags_id_v_id_c_truth->GetListOfFunctions()->Add(palette,"br");

   ci = TColor::GetColor("#ff0000");
   VbbHcc_eff_tags_id_v_id_c_truth->SetLineColor(ci);
   VbbHcc_eff_tags_id_v_id_c_truth->GetXaxis()->SetTitle("index (c_{0})");
   VbbHcc_eff_tags_id_v_id_c_truth->GetXaxis()->SetLabelFont(42);
   VbbHcc_eff_tags_id_v_id_c_truth->GetXaxis()->SetTitleOffset(1);
   VbbHcc_eff_tags_id_v_id_c_truth->GetXaxis()->SetTitleFont(42);
   VbbHcc_eff_tags_id_v_id_c_truth->GetYaxis()->SetTitle("index (c_{1})");
   VbbHcc_eff_tags_id_v_id_c_truth->GetYaxis()->SetLabelFont(42);
   VbbHcc_eff_tags_id_v_id_c_truth->GetYaxis()->SetTitleFont(42);
   VbbHcc_eff_tags_id_v_id_c_truth->GetZaxis()->SetLabelFont(42);
   VbbHcc_eff_tags_id_v_id_c_truth->GetZaxis()->SetTitleOffset(1);
   VbbHcc_eff_tags_id_v_id_c_truth->GetZaxis()->SetTitleFont(42);
   VbbHcc_eff_tags_id_v_id_c_truth->Draw("colz");
   eff_id_v_id_c_truth_tags_signal_16->Modified();
   eff_id_v_id_c_truth_tags_signal_16->cd();
   eff_id_v_id_c_truth_tags_signal_16->SetSelected(eff_id_v_id_c_truth_tags_signal_16);
}
