...
colors = [416+1, 632+1, 600+1, 400+1, 840+1, 860+1, 880+1, 900+1, 432+1, 616+1, 616+3, 800+7] #Red, green, blue, yellow, orange,cyan,magnetan 
...
def scaleToLumi1(fName, xSec, lumi,hName='Nevt'):
  f = ROOT.TFile.Open(fName, 'read')
  hTmp = f.Get(hName)
...
def scaleToLumi2(fName, xSec, lumi):
   f = ROOT.TFile.Open(fName, 'read')
   hTmp = f.Get('Nevt')
   nEvt = hTmp.GetBinContent(4)
   return lumi*xSec/(nEvt)

def customBin(h, xDiv):
  hOut = ROOT.TH1D(h.GetName() + '_customBinning','',len(xDiv)-1,array('f',xDiv))
...
def NormBinWidth(hIn):
  hOut = hIn.Clone(hIn.GetName()+'_normBinWidth')
  for i in range(1,hOut.GetNbinsX()+1):
    hOut.SetBinContent(i, hIn.GetBinContent(i)/hIn.GetBinWidth(i))
    hOut.SetBinError(i, hIn.GetBinError(i)/hIn.GetBinWidth(i))
  return hOut

def makeStackPlot(plots, plotNames_new, cName, plotDir = 'Test/', xAxisTitle = 'Jet M_{SV} [GeV]', xAxisRange = [0,10], uncName = 'MC unc. (stat.)', normMC=True, logY=False, normBinWidth = -1, legendOrder=[], minY_forLog = 0.01, lumi = '35.9'):
