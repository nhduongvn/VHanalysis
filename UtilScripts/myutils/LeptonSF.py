...
class LeptonSF:
    ...
    def get_2D(self, pt, eta):
        ...
        for etaKey, values in sorted(self.res[self.lep_binning].iteritems()) :
            #print etaKey, ' ', values
            etaL = float(((etaKey[stripForEta:]).rstrip().rstrip(']').split(',')[0]))
            etaH = float(((etaKey[stripForEta:]).rstrip().rstrip(']').split(',')[1]))
            if not (eta>etaL and eta<etaH):
                continue
            #print etaL, etaH
            for ptKey, result in sorted(values.iteritems()) :
                ptL = float(((ptKey[4:]).rstrip().rstrip(']').split(',')[0]))
                ptH = float(((ptKey[4:]).rstrip().rstrip(']').split(',')[1]))
...
"../trkEff.json":['Graph','ratio_eff_eta3_dr030e030_corr']
}
...
for j, name in jsons.iteritems():
    ...
    #weight1 = lepCorr.get_2D( eta , pt)
    
    val = weight[0]
    err = weight[1]
    print j, name[0], ': ',  val, ' +/- ', err
    #print j, name[0], ':', weight1[0], ' +/- ', weight1[1]
'''  
